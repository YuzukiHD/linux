...
static int yt8531_set_led_default_off(struct phy_device *phydev){
	int ret;
	int val;

	val = 0xe168;
    ret = ytphy_write_ext(phydev, 0xa00b, val);
    //printk("%s",__func__);
    //yt8521_soft_reset(phydev);
    //val = ytphy_read_ext(phydev,0xa00f);
    //printk("val is %x",val);
...
static int yt8531_set_led_default_blink(struct phy_device *phydev){
	int ret;
	int val;
...
static int yt8521_read_status(struct phy_device *phydev)
{
	...
	if (link) {
		link_utp = 1;
		link_mode_8521 = 1;
		yt8531_set_led_default_blink(phydev);
		yt8521_adjust_status(phydev, val, 1);
	} else {
		yt8531_set_led_default_off(phydev);
		link_utp = 0;
	}
