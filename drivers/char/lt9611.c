...
#define MIPI_RGB888 0x00
#define MIPI_YUV422 0x01

#define AC_MODE     0
#define DC_MODE     1

#define PORT_DVI 0x00
#define PORT_HDMI 0x01

#define AUDIO_I2S 0x00
#define AUDIO_SPDIF 0x01
...
struct bridge_attr{
	u8* device_name;
	u8 cbus_addr;
	struct lt9611_mipi mipi;
	struct lt9611_hdmi hdmi;
	struct lt9611_ops ops;
};
...
static int lt9611_system_init(struct i2c_client *i2c)
{
	...
	ret = lt9611_write_array(i2c, list1);
	ret = lt9611_write(i2c,0xff, 0x82);
	ret = lt9611_write(i2c,0x00, reg_lane);
	ret = lt9611_write_array(i2c, list2);
	if (ret)
...
static int lt9611_check_video_timing(struct i2c_client *i2c)
{
	...
	printk(KERN_ERR "lt9611 check_timming h_act_a, h_act_b, v_act, v_tal: %d, %d, %d, %d\n", h_act_a, h_act_b, v_act, v_tal);
...
static int lt9611_mipi_video_timing(struct i2c_client *i2c, struct video_timing *timing)
{
	...
	printk("lt9611 timming1 is: vtotal %d vact %d htotal %d hact %d \n",timing->vtotal,timing->vact,timing->htotal,timing->hact);
	printk("lt9611 timming2 is: vs %d hs %d vfp %d hfp %d \n",timing->vs,timing->hs,timing->vfp,timing->hfp);
	printk("lt9611: set video timming ok\n");
	return ret;
}
...
static void lt9611_set_pll(struct i2c_client *i2c, struct video_timing *video_format)
{
	...
	lt9611_write_array(i2c, list1);
	lt9611_write(i2c,0xff, 0x81);
	lt9611_write(i2c,0x2d, reg_2d);

	lt9611_write_array(i2c, list2);
	lt9611_write(i2c,0xff, 0x83);
	lt9611_write(i2c,0x26, 0x80 | pcr_m);
	lt9611_write(i2c,0xff, 0x82);
	lt9611_write(i2c,0xe3, pclk_div / 65536);
	lt9611_write(i2c,0xe4, pclk_div % 65536 / 256);

	lt9611_write_array(i2c, list3);
...
static int lt9611_hdmi_tx_phy(struct i2c_client *i2c)
{
	...
	if (lt9611_bridge_attr.hdmi.couple == AC_MODE)
		couple_mode = 0x73;

	if (lt9611_bridge_attr.hdmi.couple == DC_MODE)
		couple_mode = 0x31;
...
void lt9611_hdmi_tx(struct i2c_client *i2c, struct video_timing *video_format)
{
	...
	if (lt9611_bridge_attr.hdmi.audio.port == AUDIO_I2S)
		lt9611_write(i2c, 0xd7, 0x04);

	if (lt9611_bridge_attr.hdmi.audio.port == AUDIO_SPDIF)
...
static int set_output(struct i2c_client *i2c)
{
//	int status = lt9611_get_hpd_status(i2c_dev);
	lt9611_get_hpd_status(i2c_dev);
	lt9611_write(i2c, 0xff, 0x82);
	lt9611_write(i2c, 0x07, 0xff);    // clear3
	lt9611_write(i2c, 0x07, 0x3f);    // clear3
...
static irqreturn_t lt9611_hpd_inter_handle(int irq, void *dev_id)
{
	printk("alex irq lt9611--\n");
#if 0
...
{
	gpiod_direction_output(gpio_power_en, 0);

	gpiod_direction_output(gpio_rst, 0);
}
...
static int lt9611_power_on(void)
{
	...
	gpiod_direction_output(gpio_power_en, 0);
	mdelay(5);
	gpiod_direction_output(gpio_power_en, 1);
	mdelay(5);

	gpiod_direction_output(gpio_rst, 0);
	mdelay(5);
	gpiod_direction_output(gpio_rst, 1);
	mdelay(5);
...
//	struct device_node *np = dev->of_node;
	...
	struct lt9611_data *lt9611_data;


	time_type = (&timing_list[6])->format;
	video = &timing_list[6];
	i2c_dev = client;

	gpio_power_en = devm_gpiod_get(&client->dev, "lt9611-power-en",
					       GPIOD_OUT_HIGH);
	if (IS_ERR(gpio_power_en)) {
		ret = PTR_ERR(gpio_power_en);
		dev_err(dev, "cannot get gpio_power_en %d\n", ret);
		return ret;
	}

	gpio_rst = devm_gpiod_get(&client->dev, "lt9611-rst",
					       GPIOD_OUT_HIGH);
	if (IS_ERR(gpio_rst)) {
		ret = PTR_ERR(gpio_rst);
	...
	ret = devm_request_threaded_irq(&client->dev, client->irq,
					NULL, lt9611_hpd_inter_handle, irq_flags,
					client->name, lt9611_data);
