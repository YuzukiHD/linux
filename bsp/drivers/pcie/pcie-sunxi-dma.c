...
static u32 DMA_DEFAULT_CNT = 0x04;  /* Default value for DMA wr/rd channel */

static u32 DMA_WR_CHN_CNT;
static u32 DMA_RD_CHN_CNT;
...
EXPORT_SYMBOL_GPL(pcie_dma_chan_release);
...
EXPORT_SYMBOL_GPL(pcie_dma_get_chan);
...
{
	memset(dma_wr_chn, 0, sizeof(dma_channel_t)*DMA_WR_CHN_CNT);
	memset(dma_rd_chn, 0, sizeof(dma_channel_t)*DMA_RD_CHN_CNT);
