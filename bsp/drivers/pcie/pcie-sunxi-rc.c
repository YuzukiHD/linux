...
static void sunxi_pcie_prog_outbound_atu(struct pcie_port *pp, int index, int type,
...
EXPORT_SYMBOL_GPL(sunxi_pcie_setup_rc);

static int sunxi_pcie_wait_for_speed_change(struct sunxi_pcie *pci)
{
	u32 tmp;
...
static void sunxi_plat_pcie_host_init(struct pcie_port *pp)
{
	...
	if (gpio_is_valid(pci->prsnt_gpio)) {
		gpio_direction_output(pci->prsnt_gpio, 0);
		__gpio_set_value(pci->prsnt_gpio, 0);
	} else {
		dev_err(pp->dev, "Failed to setreset gpio \n");
	}
	...
	if (gpio_is_valid(pci->prsnt_gpio)) {
		gpio_direction_output(pci->prsnt_gpio, 1);
		__gpio_set_value(pci->prsnt_gpio, 1);
	} else {
		dev_err(pp->dev, "Failed to setreset gpio\n");
	}
...
int sunxi_plat_add_pcie_port(struct pcie_port *pp,
...
EXPORT_SYMBOL_GPL(sunxi_plat_add_pcie_port);
