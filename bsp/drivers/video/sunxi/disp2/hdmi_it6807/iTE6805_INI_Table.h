...
iTE6805_INI_REG _CODE iTE6805_INIT_HDMI_TABLE[] = {
	...
	{0x22, 0xFF,
	 0x17}, /* [4]Auto Video Reset [2]Int Reset [1]Audio Reset [0]Video Reset */
	{0x23, 0xFF, 0x1F}, /* Port 0 [4]EQ Reset [3]CLKD5 Reset [2]CDR Reset
						   [1]HDCP Reset [0]All logic Reset */
	{0x2B, 0xFF, 0x1F}, /* Port 1 [4]EQ Reset [3]CLKD5 Reset [2]CDR Reset
						   [1]HDCP Reset [0]All logic Reset */
	{0x24, 0xFF, 0xF8}, /* [7:3] MHL Logic reset */
						/* {0xC5, 0xFF, 0x13}, // EDID Reset */
	{0x22, 0xFF,
	 0x10}, /* [4]Auto Video Reset [2]Int Reset [1]Audio Reset [0]Video Reset */
	{0x23, 0xFF, 0xA0}, /* Port 0 [4]EQ Reset [3]CLKD5 Reset [2]CDR Reset
						   [1]HDCP Reset [0]All logic Reset */
	{0x2B, 0xFF, 0xA0}, /* Port 1 [4]EQ Reset [3]CLKD5 Reset [2]CDR Reset
						   [1]HDCP Reset [0]All logic Reset */
	...
	{0xAC, 0xFF, 0x40},
	{0x0F, 0xFF, 0x00},
	{0x3A, 0xFF, 0x89}, /* 0406 add, 0421 modify, emily command for CED Error
						   sensetive from 89 to 99 */
	...
	{0x6E, 0xFF, 0x80},
	{0x77, 0xFF,
	 0x87}, /* Reg_PktRec_Type, set from 83 to 87 for record DRM pkt */
	...
	{0xCA, 0xFF, 0x99}, /* 0106 modify Richard suggest 5mA, slow */
						/* 0113 add start */
	{0x0F, 0xFF, 0x00},
	{0x86, 0x0C, 0x08}, /* 0118 modify, HW RD mail [IT6664, IT6805, IT6634
						   Audio Seeting note] by Andrew Chang */
	...
	{0xE3, 0xFF,
	 0x07}, /* 0426 modify by RD code edit it from 06 to 05, recommand by
			   emily, and 11/24 modify for B1 and later */
	...
	{0xE3, 0xFF, 0x07},	 /* 0426 modify by RD code edit it from 06 to 05,
							recommand by emily */
	{0x27, 0xFF, EQ_INIT_VALUE},
	{0x28, 0xFF, EQ_INIT_VALUE},
	{0x29, 0xFF, EQ_INIT_VALUE},
...
#if (iTE68052 == 1)
	{0xC4, 0xFF,
	 0x1B}, /* change by mail '[IT68052] Action for modification' */
	{0xCC, BIT1,
	 BIT1}, /* change by mail '[IT68052] Action for modification' */
#endif

#if (iTE6807 == 1)
	{0xC7, 0xFF,
	 0x49}, /* change by mail '[IT68052] Action for modification' */
#endif

	{BANK, BANKM, 0x00}, /* change bank 5 */

	{BANK, BANKM, 0x04}, /* change bank */
	/* add for MHL port0 plug-unplug port1 screen will be Flashing. by emily
	   comment 0721 'IT6805 P0 MHL + P1 HDMI' */
	{0x3C, BIT5, 0},
	...
	/* 0xCE 4: Enable Fast authentication 5: Enable feature 1.1 need to be set
	   to 0 */
	{0xCE, 0x30, 0x00},
	{BANK, BANKM, 0x04},
	{0xCE, 0x30, 0x00},
	{0x42, 0xE0, 0xC0}, /* add 20180313 by emily mail, 20190313 ADD Reg42[5] =
						   0 for 8K1K resolution */
	{BANK, BANKM, 0x00},
	{0x42, 0xE0, 0xC0}, /* add 20180313 by emily mail */
	{0x7B, BIT4, BIT4}, /* for detect DRM PKT */

#if defined(ENABLE_ACP_REGISTER_TO_DETECT_AVI_INFOFRAME) && \
	(ENABLE_ACP_REGISTER_TO_DETECT_AVI_INFOFRAME == 1)
	{0x78, 0xFF, 0x82}, /* Change from ACP to AVI Infoframe detect for more AVI
						   information */
#endif
	/* mail need to set to all 6805 Ax/Bx chip by mail IT6805 SW register
	   setting */
	{0x3C, 0x21, 0x00},
