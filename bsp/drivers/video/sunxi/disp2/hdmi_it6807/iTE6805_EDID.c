...
#ifdef _HDMI_SWITCH_
iTE_u8 txphyadr[2]; /* for CEC function (TX Reveice EDID ABCD transfrom to RX
					   EDID ABCD phy Value, only for HDMI Switch) */
#endif
...
void iTE6805_EDID_Init(void)
{
	...
	hdmirxwr(
		0x4B,
		(ADDR_EDID |
		 0x01)); /* [7:1] EDID RAM Slave Adr ,[0]1: Enable access EDID block */
...
void iTE6805_EDID_RAMInitial(void)
{
	...
	PORT0_EDID_Block1_CheckSum = (Block1_CheckSum + piTE6805EDID[VSDB_ADDR] +
								  piTE6805EDID[VSDB_ADDR + 1] -
								  rxphyadr[PORT0][0] - rxphyadr[PORT0][1]) %
								 0x100;
	PORT1_EDID_Block1_CheckSum = (Block1_CheckSum + piTE6805EDID[VSDB_ADDR] +
								  piTE6805EDID[VSDB_ADDR + 1] -
								  rxphyadr[PORT1][0] - rxphyadr[PORT1][1]) %
								 0x100;
	...
	for (offset = (AddStart + 0x04); offset < (AddStart + End);) {
		...
		case 0x03:
			...
			if ((*(pEDID + offset)) == 0x03 &&
				...
				VSDB_ADDR = offset + 3;
#if (_ENABLE_MHL_SUPPORT_3D_ == TRUE)
				/* Paser EDID 3D block in VSDB if it have for using in MSC
				 * Command */
				...
				if (ucTemp & 0x80) /* If it have 'Video and Audio Latency
									  present', need add offset */
					ADDR_3DPresent += 2;

				if (ucTemp & 0x40) /* If it have 'Interlaced Video and Audio
									  Latency present', need add offset */
					ADDR_3DPresent += 2;

				/* If it have 'HDMI additional video format present' need add
				 * offset */
				if (ucTemp & 0x20) {
					ADDR_3DPresent++;
				}
				...
				if ((*(pEDID + ADDR_3DPresent)) & 0x80) {
					/* 3D_present = 1 = EDID support 3D , only it condition
					 * needed support 3D */
...
#if FALSE	/* mark useless block */
			/* HDMI Video Data Block (include one or more Short Video
			 * Description) */
		case 0x02:
			iTE6805_EDID_Parse3D.ucVicCnt = length;
			for (i = 0; i < length; i++) {
				ucTemp = (*(pEDID + offset + i)) & 0x7F;
				iTE6805_EDID_Parse3D.SVD_LIST[i] =
					ucTemp; /* short video decription */
			}
			break;
#endif
		default:
...
void iTE6805_EDID_Set_Phyaddress(void)
{
	...
	EDID_DEBUG_PRINTF("\nDnStrm PhyAdr = %X, %X, %X, %X\n", (int)txphyA,
					  (int)txphyB, (int)txphyC, (int)txphyD);
	...
	EDID_DEBUG_PRINTF(" PortA PhyAdr = %X, %X, %X, %X\n", (int)rxphyA,
					  (int)rxphyB, (int)rxphyC, (int)rxphyD);
...
void iTE6805_EDID_ParseVSDB_3Dblock(void)
{
	...
	iTE_u8 ucRdPtr =
		iTE6805_EDID_Parse3D.uc3DEdidStart; /* start from 3D_present address */
	...
	/* Re-initial bVSDBspport3D =0 then check with 3D_Structure and 3D_MASK at
	 * HDMI VSDB block of EDID */
	...
	for (; ucRdPtr <= uc3DEdidEnd;) {
		...
		case PARSE3D_LEN:
			...
			ucRdPtr +=
				(Customer_EDID[iTE6805_DATA
							  ...
							  [ucRdPtr] >>
				 5) +
				1; /* Parse 3D struct, Add CurrentPtr to VIC End */
		...
		case PARSE3D_STRUCT_L:
			ucTemp = Customer_EDID[iTE6805_DATA
									   .US_EDID_INDEX[iTE6805_DATA.CurrentPort]]
								  [ucRdPtr++];
			...
			uc3DMulti &=
				MHL_3D_Mask; /* set 3D Multi to 0, only left 3D_Support Info */

			if ((uc3DMulti & 0x60) == 0x20) {
				e3DEdidState = PARSE3D_VIC;
				/* If 3D_Multi_present = 01 is present and assigns 3D formats
				 * to all of the VICs listed */
				/* in the first 16 entries in the EDID. 3D_MASK_15 is not
				 * present. */
		...
		case PARSE3D_MASK_H:
			if (Customer_EDID[iTE6805_DATA
								  .US_EDID_INDEX[iTE6805_DATA.CurrentPort]]
							 [ucRdPtr]) {
				iTE6805_EDID_Parse3D.bVSDBspport3D =
					0x01; /* for identify 3D_MASK have Short Video Descriptor
							 (SVD) support 3D format */
			}
		...
		case PARSE3D_MASK_L:
			if (Customer_EDID[iTE6805_DATA
							 ...
							 [ucRdPtr]) {
				iTE6805_EDID_Parse3D.bVSDBspport3D =
					0x01; /* for identify 3D_MASK have SVD support 3D format */
			}
