...
void iTE6805_Inform_HDCP1_KSVListReady_CB(unsigned char *BKSV,
...
void iTE6805_Inform_HDCP2_KSVListReady_CB(unsigned char *RxID,
										  unsigned char *RxInfo,
										  unsigned char *RxID_List)
{
	iTE_u8 i, dev_count, depth;
...
void iTE6805_Inform_EDIDReadyBlock1_CB(unsigned char *EDID)
{
	...
	if (VSDB_ADDR == 0) {
		...
		PORT0_EDID_Block1_CheckSum =
			(Block1_CheckSum + EDID[VSDB_ADDR] + EDID[VSDB_ADDR + 1] -
			 iTE6805_DATA.rxphyadr[PORT0][0] -
			 iTE6805_DATA.rxphyadr[PORT0][1]) %
			0x100;
		...
		hdmirxwr(0xCA,
				 PORT0_EDID_Block1_CheckSum); /* Port 0 Block 1 CheckSum */
	}
...
{
	HDCP_DEBUG_PRINTF(
		"PRT : iTE6805_Inform_EDIDReadyNoNeedUpdate_CB called \r\n");
	if (iTE6805_DATA.ENABLE_6805_AS_HDCP_REPEATER == 0) {
		return;
	}
	iTE6805_DATA.Flag_EDIDReady = 1;
...
{
	HDCP_DEBUG_PRINTF(
		"PRT : iTE6805_Inform_HDCP_EnableRepeaterMode called \r\n");
	if (Enable) {
