...
iTE6805_INI_REG _CODE iTE6805_INIT_MHL_TABLE[] = {
	...
	{0x2A, 0xFE,
	 (Reg5VStableTSel << 1) + (RegWKpulseLAdj << 3) + (RegWKpulseSAdj << 4) +
		 (RegWKFailThr << 5) + (RegDisvOpt << 7)},
	{0x2B, 0x3E,
	 (RegAutoWakeUpFailTrg << 1) + (RegEnWakeUpFailTog << 2) +
		 (RegEnWakeUpFailFlt << 3) + (RegWakeUpFailTogSel << 4)},

	{0x52, 0xFF, 0x00}, /* 0: handle by HW */
	{0x53, 0xFF, 0x80}, /* [7]=1 disable FW mode */
	{0x01, 0x8F, 0x88}, /* Cbus configure , [6:4] OSCCLK Divide count select = 5
						   for bit time =1us */
	/* {0x01, 0xFF, (EnCBusDeGlitch<<7) | (Reg100msTOAdj<<2) }, /* Cbus
	   configure , [6:4] OSCCLK Divide count select = 5 for bit time =1us */
	{0x0C, 0x89,
	 (PPHDCPOpt << 7) + EnHWPathEn + (PPHDCPOpt2 << 3)}, /* PATH_EN configure */
	...
	{0x5C, 0xFF, 0xD4},
	/* {0x5C, 0xFC, (EnPktFIFOBurst<<7) + (MSCBurstWrID<<6) + (MSCBurstWrOpt<<5)
	   + (EnMSCBurstWr<<4) + (EnMSCHwRty<<3) + (MSCRxUCP2Nack<<2)}, */
	{0x66, 0xFF, 0x02},
