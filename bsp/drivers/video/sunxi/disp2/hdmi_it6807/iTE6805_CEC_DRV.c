...
iTE_u8 iTE6805_CEC_RX_CMD_Push(piTE_u8 pHeader)
{
	...
	iTE_u8 u8CecSize =
		(pHeader[2] & 0x1F) + 1; /* Reg4F[0:4] -> Command FIFO In_Cnt */
	iTE_u8 u8FreeSize;
	iTE_u8 u8Temp;
...
iTE_u8 iTE6805_CEC_RX_CMD_Pull(void)
{
	...
	CMD_Size = pu8CmdBuf[0] =
		pRX_Queue[Rptr++]; /* set pu8CmdBuf = CEC_FRAME.id.CMD_SIZE */

	if (CMD_Size < 3) {
		/* Size+Header+OpCode = 3 */
		/* only polling is 2, but will be trigger RX Fail INT, so if we get < 3
		 * CMD, that's an error */
	...
	pu8CmdBuf[0]--;
	iTE6805_CEC->CEC_FRAME_RX.id.Follower =
		iTE6805_CEC->CEC_FRAME_RX.id.HEADER & 0x0F;
	iTE6805_CEC->CEC_FRAME_RX.id.Initiator =
		iTE6805_CEC->CEC_FRAME_RX.id.HEADER >> 4;
	CEC_DEBUG_PRINTF("CEC Decoding -> Initiator = 0x%02X, Follower = 0x%02X \n",
					 (int)iTE6805_CEC->CEC_FRAME_RX.id.Initiator,
					 (int)iTE6805_CEC->CEC_FRAME_RX.id.Follower);
