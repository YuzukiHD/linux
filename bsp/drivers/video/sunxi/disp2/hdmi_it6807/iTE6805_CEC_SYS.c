...
_CODE iTE_u8 cec_msg_size[256] = {
	,
...
iTE_s32 iTE6805_hdmirx_CEC_irq(void)
{
	/* 0x4C */
	/* [5] TX Fail */
	/* [4] RX Done */
	...
	if (Reg4Ch & BIT5) {
		...
		ret = iTE6805_CEC_chg(STATECEC_TXFail);
	}
	...
	if (Reg4Ch & 0xC0) {
		...
		CEC_DEBUG_PRINTF(
			"INT DataOvFlw_Int or CmdOvFlw_Int need to "
...
iTE_s32 iTE6805_CEC_chg(STATECEC_Type NewState)
{
	iTE_s32 ret = 0;
	...
	case STATECEC_TXFail:
		...
		if (!iTE6805_CEC_Check_IsPollingMsg()) {
			...
			if (iTE6805_CEC_Get_TxFailStatus() == TX_FAIL_RECEIVE_NACK) {
				if ((Current_TX_Retry_Count++ < Max_TX_Retry_Count) &&
					iTE6805_CEC_Check_Fire_Ready()) {
					iTE6805_CEC_Fire_Tx();
				}
			}
		}
	...
	case STATECEC_TXDone:
		...
		if (iTE6805_CEC_Check_IsPollingMsg()) {
			if (iTE6805_CEC_Get_TxFailStatus() == TX_FAIL_RECEIVE_ACK) {
				...
				/* but 6805 is sink (CEC_LOG_ADDR_TV), do not need to do
				 * anything */
				/* polling will trigger TX Fail, so in Reg4D, 4E, 4F will 100%
				 * get not-polling-message. */
...
void iTE6805_CEC_CMD_Ready_To_Fire(void)
{
	...
	cecwr(0x23,
		  iTE6805_CEC->CEC_FRAME_TX.id.CMD_SIZE); /* write size to register */
	cecbwr(0x10, iTE6805_CEC->CEC_FRAME_TX.id.CMD_SIZE,
		   &iTE6805_CEC->CEC_FRAME_TX.CEC_ARY[1]); /* write CMD to register */
...
void iTE6805_CEC_CMD_Push_To_Queue_Handler(void)
{
	...
	do {
		if (pu8Reg51[0] & 0xCC) {
			...
			while (RxCMDStage--) {
				cecbrd(0x4D, 0x03,
					   pu8RxHeader); /* Read 4D --> Header, OpCode, In_Cnd */
				CEC_DEBUG_PRINTF("0x%02X, %02X, %02X\n", (int)pu8RxHeader[0],
								 (int)pu8RxHeader[1], (int)pu8RxHeader[2]);
				if (FALSE == iTE6805_CEC_RX_CMD_Push(pu8RxHeader)) {
...
iTE_u8 iTE6805_CEC_CMD_Check(pCEC_FRAME CEC_FRAME)
{
	...
	if (CEC_FRAME->id.CMD_SIZE <= 1 ||
		CEC_FRAME->id.CMD_SIZE > CEC_MAX_MSG_SIZE ||
		CEC_FRAME->id.CMD_SIZE < Length_Min) {
		CEC_DEBUG_PRINTF("CEC CMD Check Size Fail!\n");
		Flag_Return = FALSE;
	}

	/* I'm the Initiator but I get the command */
	if (CEC_FRAME->id.Initiator != CEC_LOG_ADDR_UNREGISTERED &&
		iTE6805_CEC->MY_LA == CEC_FRAME->id.Initiator) {
	...
	if (IS_BOARD_CAST != TRUE && iTE6805_CEC->MY_LA != CEC_FRAME->id.Follower) {
		CEC_DEBUG_PRINTF(
			"CEC Check if this message was for us (directed or broadcast) "
			"Fail!\n");
		Flag_Return = FALSE;
	}
	...
	if (cec_msg_size[CEC_FRAME->id.OPCODE]) {
		if (!IS_BOARD_CAST &&
			!(cec_msg_size[CEC_FRAME->id.OPCODE] & DIRECTED)) {
			CEC_DEBUG_PRINTF(
				"CEC Check Direction Fail (Need to be Boardcast)!\n");
			Flag_Return = FALSE;
		} else if (IS_BOARD_CAST &&
				   !(cec_msg_size[CEC_FRAME->id.OPCODE] & BCAST)) {
			CEC_DEBUG_PRINTF(
				"CEC Check Direction Fail! (Need to be Directed)!\n");
			Flag_Return = FALSE;
		}
	}
...
void iTE6805_CEC_INIT(void)
{
	...
	iTE6805_CEC_RX->MY_PA_LOW =
		0x00; /* ROOT TV PA FROM HIGH TO LOW = 0.0.0.0 */
	...
	iTE6805_CEC = &iTE6805_CEC_RX[0]; /* point to CEC_RX, if needed TX, need to
										 implement change TX RX sel */
