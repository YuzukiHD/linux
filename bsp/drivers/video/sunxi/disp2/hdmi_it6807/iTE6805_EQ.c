...
_CODE iTE_u8 RS_Value[] = {0x7F, 0x7E, 0x3F, 0x3E, 0x1F, 0x1E, 0x0F,
...
_CODE iTE_u8 iTE6805_EQ_Table_Hdmi10[] = {
	0x01, 0x3F, 0x07, 0x0F}; /* hdmi1.x from week to strong */
_CODE iTE_u8 iTE6805_EQ_Table_Hdmi20[] = {
	0x01, 0x07, 0x0F, 0x3F}; /* hdmi2.x from strong to week */

iTE_u8 CED_Err[Table_CED_Max_Index]
			  [6]; /* index 0 for autoEQ, 1:4 are Manual EQ */
...
iTE_u8 Current_EQ_TableIndex; /* fix for cross complier warning, array index
								 should be unsigned */
...
void iTE6805_hdmirx_port0_EQ_irq(void)
{
	...
	if (Reg07h != 0x00) {
		...
		if (Reg07h & 0x10) {
			...
			/* 07/10 emily command add register setting trigger-eq to 0 prevent
			 * some unstable condition */
...
void iTE6805_EQ_fsm(void)
{
	...
	/* if MHL Mode */
	if (iTE6805_DATA.CurrentPort == PORT0 &&
		iTE6805_Check_PORT0_IS_MHL_Mode(PORT0)) {
	...
	case STATEEQ_ClockONDet_And_TriggerEQ:
		if (iTE6805_Check_CLK_Vaild()) {
			EQ_DEBUG_PRINTF(
				"******EQ in STATEEQ_ClockONDet_And_TriggerEQ state and "
				"iTE6805_Check_CLK_Vaild == TRUE ******\n");
			...
			if (iTE6805_Check_HDMI2() == TRUE) {
				...
				/* for EQ effectiency, clk valid then check BitErr judge if
				 * AutoEQ or not */
				...
				iTE6805_Trigger_EQ(); /* Trigger then wait for EQ Done
										 interrupt */
			}
	...
	case STATEEQ_EQWaitAutoEQUntilDone:
		EQ_DEBUG_PRINTF(
			"******EQ iTE6805_EQ_fsm STATEEQ_EQWaitAutoEQUntilDone state "
			"******\n");
		...
		if (Current_AutoEQ_Timeout_Count > EQ_AutoEQ_Timeout ||
			...
			Flag_Fail_SAREQ = FALSE;
			EQ_DEBUG_PRINTF(
				"******EQ Can't wait EQ Done INT or EQ Fail Int, change to "
				"Manual EQ ******\n");
	...
	case STATEEQ_EQCheckAutoEQDone:
		EQ_DEBUG_PRINTF(
			"******EQ iTE6805_EQ_fsm STATEEQ_EQCheckAutoEQDone state ******\n");
		break;
	...
	case STATEEQ_EQDone:
		...
		if (Current_EQ_Report_BirErr_Count == 0x00) {
			...
			if (iTE6805_Check_HDMI2() == FALSE) {
				...
				if (iTE6805_BitErr_Check_Again() == FALSE) {
					EQ_DEBUG_PRINTF(
						"******EQ Done But Find CED Error in EQDone Stage, "
						"Restart Manual EQ ******\n");
		...
		if (Current_EQ_Report_BirErr_Count ==
...
void iTE6805_EQ_chg(STATEEQ_Type NewState)
{
	...
	/* If ECC Error need to reset HPD, this code for ignore EQ_chg when ECC
	 * Reset HPD until 5V off */
	...
	if (iTE6805_DATA.CurrentPort == PORT0 &&
		...
		if (iTE6805_DATA.STATEEQ != STATEEQ_EQDone ||
			...
			if (iTE6805_DATA.STATEEQ != STATEEQ_EQDone) {
				/* MHL do not need to do EQ, only using fix EQ 9F, recommand by
				 * JJ */
				EQ_DEBUG_PRINTF(
					"******MHL Mode Change To EQDone State, do not need to do "
					"EQ, change to EQDone Stage ******\n");
				iTE6805_DATA.STATEEQ = STATEEQ_EQDone;
				return;
			}
		}
	...
	case STATEEQ_Off:
		...
		break;
	case STATEEQ_ClockONDet_And_TriggerEQ:
		/* 20170411 Flag_Need_Set_CKITPENI add from emily mail 'it66341 ff
		 * corner lot issue', when input > 1G, Reg3A7_bit6[Reg_P0_CKITPENI] = 1
		 */
...
#if (_ENABLE_EXTERN_EQ_CTRL_ == TRUE)
		if (iTE6805_DATA.EQ_Customer_Setting[iTE6805_DATA.CurrentPort] !=
			EQ_AUTO) {
			iTE6805_DATA.STATEEQ = STATEEQ_KeepEQStateUntil5VOff;
			EQ_DEBUG_PRINTF(
				"******EQ change to STATEEQ_KeepEQStateUntil5VOff state "
				"because Customer not set to EQ_AUTO ******\n");
		}
#endif
		break;
	case STATEEQ_EQWaitAutoEQUntilDone:
	...
	case STATEEQ_EQCheckAutoEQDone:
		EQ_DEBUG_PRINTF(
			"******EQ change to STATEEQ_EQCheckAutoEQDone state ******\n");
		if (Flag_Done_EQ) {
			iTE6805_Report_EQ();
		...
		if (Flag_Done_SAREQ) {
			...
			if (Flag_Need_Trigger_RSSKEW == TRUE &&
				(iTE6805_DATA.ChipID != 0xA0 && iTE6805_DATA.ChipID != 0xA1))
#else
			if (Flag_Need_Trigger_RSSKEW == TRUE)
#endif
			{
			...
			{
				EQ_DEBUG_PRINTF(
					"****** ChipID=0xB0 iTE6805_Report_Skew ******\n");
				iTE6805_Report_Skew();
			}
			...
			if (Flag_Need_Check_EQResult == TRUE) {
				...
				if (Flag_AMP_ALL_Timeout == FALSE) {
					...
					if (iTE6805_DATA.CurrentPort == PORT0)
						...
						chgbank(7);
					hdmirxset(0x22, 0x40,
							  0x40); /* Enable ENDFE, DEF value is cauclate by
										AutoEQ, Disable when AutoEQ, Enable
										after AutoEQ */
				} else {
					if (iTE6805_DATA.CurrentPort == PORT0)
						chgbank(3);
	...
	case STATEEQ_EQCheckBitErr:
		if (iTE6805_BitErr_Check()) {
			if (Flag_Need_Trigger_RSSKEW == TRUE) {
				EQ_DEBUG_PRINTF(
					"******EQ Check OK, do not need to trigger "
					"RSSKEWEQ******\n");
				Flag_Need_Trigger_RSSKEW = FALSE;
			}
#ifdef EQ_KURO_TEST
		...
		Force_EQ_Fail:
#endif
			if (CED_Err[Current_EQ_TableIndex][1] == 0x00 &&
				CED_Err[Current_EQ_TableIndex][3] == 0x00 &&
				CED_Err[Current_EQ_TableIndex][5] == 0x00) {
				EQ_DEBUG_PRINTF(
					"               Flag_Need_to_Force_SCDC_Clock_Ratio \n");
				...
				if (iTE6805_Check_HDMI2()) {
					/* SCDC Clock Ratio = 1/40  ->  Force Change to 1/10 and
					 * disable scramble */
					if (iTE6805_DATA.CurrentPort == PORT0)
						chgbank(3);
					else if (iTE6805_DATA.CurrentPort == PORT1)
						chgbank(7);
					hdmirxset(0xE5, 0x1C,
							  0x10); /* set clock = 1/10 and disable scramble */
					...
					if (iTE6805_DATA.CurrentPort == PORT0)
						...
						chgbank(7);
					hdmirxset(0xE5, 0x1C,
							  0x1C); /* set clock = 1/40 and enable scramble */
					EQ_DEBUG_PRINTF(
						"               set clock = 1/40 and enable scramble "
						"\n");
				}
				chgbank(0);
				iTE6805_EQ_chg(STATEEQ_Off);
			...
			if (Flag_Need_Trigger_RSSKEW == TRUE) {
				EQ_DEBUG_PRINTF(
					"****** ChipID=0xA0/A1 Flag_Need_Trigger_RSSKEW ******\n");
	...
	case STATEEQ_EQManual:
		...
		if (Current_EQ_TableIndex < Table_CED_Max_Index) {
			...
			/* exceed Max Index of EQ Table, none of all EQ < Threshold, can't
			 * be EQ Done */
			/* then need Manual EQ + RSSKEW */
			Current_EQ_TableIndex =
				0; /* using Current_EQ_TableIndex = 0 for index or exceed */
			EQ_DEBUG_PRINTF(
				"******EQ STEP 5(Final) . Judge Best EQ and Check "
				"BigErr******\n");
			...
			for (i = 0; i < Table_CED_Max_Index - 1; i++) {
				if (gEQ_B_BitErr_Valid == FALSE && (CED_Err[i + 1][1] & 0x80)) {
					...
					if (CED_B_TEMP < CED_B) {
						...
						CED_B = CED_B_TEMP;
						EQ_DEBUG_PRINTF(
							"**EQ Find Best B RS (BestEQValue) = 0x%02X, CED "
							"Err = 0x%04X **\n",
				...
				if (gEQ_G_BitErr_Valid == FALSE && (CED_Err[i + 1][3] & 0x80)) {
					...
					if (CED_G_TEMP < CED_G) {
						...
						CED_G = CED_G_TEMP;
						EQ_DEBUG_PRINTF(
							"**EQ Find Best G RS (BestEQValue) = 0x%02X, CED "
							"Err = 0x%04X **\n",
				...
				if (gEQ_R_BitErr_Valid == FALSE && (CED_Err[i + 1][5] & 0x80)) {
					...
					if (CED_R_TEMP < CED_R) {
						...
						CED_R = CED_R_TEMP;
						EQ_DEBUG_PRINTF(
							"**EQ Find Best R RS (BestEQValue) = 0x%02X, CED "
							"Err = 0x%04X **\n",
			...
			if (gEQ_B_BitErr_Valid == FALSE)
				EQ_DEBUG_PRINTF(
					"****EQ Force B RS value (BestEQValue) to 0x%02x ****\n",
					(int)gEQ_B_TargetRS);
			if (gEQ_G_BitErr_Valid == FALSE)
				EQ_DEBUG_PRINTF(
					"****EQ Force G RS value (BestEQValue) to 0x%02x ****\n",
					(int)gEQ_G_TargetRS);
			if (gEQ_R_BitErr_Valid == FALSE)
				EQ_DEBUG_PRINTF(
					"****EQ Force R RS value (BestEQValue) to 0x%02x ****\n",
					(int)gEQ_R_TargetRS);

			if (iTE6805_Check_HDMI2()) {
				EQ_DEBUG_PRINTF(
					"***EQ Trigger iTE6805_Trigger_RSSKEW_EQ() in Final Step "
					"***\n");
				Flag_Need_Trigger_RSSKEW = TRUE;
				iTE6805_Trigger_RSSKEW_EQ();
				iTE6805_EQ_chg(STATEEQ_EQWaitAutoEQUntilDone);
...
#endif
					EQ_DEBUG_PRINTF(
						"***EQ iTE6805_EQ_chg(STATEEQ_WaitForManualEQ) ***\n");
...
iTE_u8 iTE6805_BitErr_Check(void)
{
...
#ifndef EQ_KURO_TEST
	if (gEQ_B_BitErr_Valid == FALSE &&
		CED_Err[Current_EQ_TableIndex][1] != 0xFF &&
		(CED_Err[Current_EQ_TableIndex][1] == 0x80 &&
		 CED_Err[Current_EQ_TableIndex][0] < (iTE_u8)EQ_BitErr_Threshold))
		gEQ_B_BitErr_Valid = TRUE;

	if (gEQ_R_BitErr_Valid == FALSE &&
		CED_Err[Current_EQ_TableIndex][5] != 0xFF &&
		(CED_Err[Current_EQ_TableIndex][5] == 0x80 &&
		 CED_Err[Current_EQ_TableIndex][4] < (iTE_u8)EQ_BitErr_Threshold))
	...
	if (iTE6805_DATA.ChipID != 0xA0) {
		...
		if (gEQ_G_BitErr_Valid == FALSE &&
			...
			(CED_Err[Current_EQ_TableIndex][3] == 0x80 &&
			 CED_Err[Current_EQ_TableIndex][2] < (iTE_u8)EQ_BitErr_Threshold))
			gEQ_G_BitErr_Valid = TRUE;
	} else {
		if (gEQ_G_BitErr_Valid == FALSE &&
			CED_Err[Current_EQ_TableIndex][3] != 0xFF &&
	...
	if (gEQ_B_BitErr_Valid == TRUE && gEQ_G_BitErr_Valid == TRUE &&
		gEQ_R_BitErr_Valid == TRUE) {
		if (iTE6805_DATA.CurrentPort == PORT0)
			chgbank(3);
		else if (iTE6805_DATA.CurrentPort == PORT1)
...
iTE_u8 iTE6805_BitErr_Check_Again(void)
{
	...
	if (CED_Err[Current_EQ_TableIndex][1] != 0x80 ||
		CED_Err[Current_EQ_TableIndex][0] > EQ_BitErr_Threshold) {
	...
	if (iTE6805_DATA.ChipID != 0xA0 &&
		(CED_Err[Current_EQ_TableIndex][3] != 0x80 ||
		 CED_Err[Current_EQ_TableIndex][2] > EQ_BitErr_Threshold))
#else
	if (CED_Err[Current_EQ_TableIndex][3] != 0x80 ||
		CED_Err[Current_EQ_TableIndex][2] > EQ_BitErr_Threshold)
#endif
	{
		gEQ_G_BitErr_Valid = FALSE;
	...
	if (CED_Err[Current_EQ_TableIndex][5] != 0x80 ||
		CED_Err[Current_EQ_TableIndex][4] > EQ_BitErr_Threshold) {
...
#if (iTE68051 == TRUE || iTE68052 == TRUE)
	if (iTE6805_DATA.ChipID == 0xA0 &&
		(gEQ_B_BitErr_Valid == FALSE || gEQ_R_BitErr_Valid == FALSE)) {
		gEQ_G_BitErr_Valid = FALSE;
		Result = FALSE;
	}
#endif
...
iTE_u8 iTE6805_BitErr_Check_For_Parity_Error(void)
{
	...
	if (Reg1 == 0xBF && Reg2 == 0x38 && Reg3 == 0x80 && Reg4 == 0x00 &&
		Reg5 == 0x00) {
#ifdef EQ_KURO_TEST
		return FALSE;
#endif
...
void iTE6805_Trigger_SAREQ(void)
{
	...
	hdmirxwr(0x39, 0x0D); /* [5] POLBOPT, [4] ADDPClrOPT, [3:0] CED Valid
							 Threshold 0x0D */
...
void iTE6805_Reset_EQ(void)
{
	...
	if (iTE6805_DATA.CurrentPort == PORT0) {
		...
		if (iTE6805_DATA.EQ_Customer_Setting[PORT0] != 0x00) {
			EQ_DEBUG_PRINTF(
				"PORT1 set 0x27 = iTE6805_DATA.EQ_Customer_Setting[PORT0]= "
				"0x%02X \n",
	...
	if (iTE6805_DATA.CurrentPort == PORT1) {
		...
		if (iTE6805_DATA.EQ_Customer_Setting[PORT1] != 0x00) {
			EQ_DEBUG_PRINTF(
				"PORT1 set 0x27 = iTE6805_DATA.EQ_Customer_Setting[PORT1]= "
				"0x%02X \n",
...
void iTE6805_Set_EQResult_Flag(void)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (AMP_TimeOut[i] == 0x3FFF) {
			Flag_AMP_ALL_Timeout =
				TRUE; /* Channel Timeout, ignore ALL DFE value setting */
			EQ_DEBUG_PRINTF(
				"CH %d, ALL_TimeOut, set Flag_AMP_ALL_Timeout = TRUE \n",
				(int)i);
		}
...
void iTE6805_Report_Skew(void)
{
	...
	for (Rec_Channel = 0; Rec_Channel < 3; Rec_Channel++) {
		...
		EQ_DEBUG_PRINTF("\n SKEW info of Selected channel: %d : \n",
						(int)Rec_Channel);
		EQ_DEBUG_PRINTF(
			" SKEW00 : BErr =%d, SKEWP0 : BErr =%d, SKEWP1 : BErr =%d \n\n",
...
void iTE6805_BitErr_Get(void)
{
	...
	if (iTE6805_DATA.CurrentPort == PORT0)
		...
		chgbank(4);
	hdmirxset(0x3B, 0x08,
			  0x08); /* read CED Error from SAREQ CEDError Counter */
	...
	for (i = 0; i < 0x01; i++) {
		...
		if (iTE6805_DATA.STATEEQ == STATEEQ_EQDone) {
			...
			CED_Err_Total[Channel_B] +=
				(CED_Err[Current_EQ_TableIndex][1] & 0x7F) << 8;

			CED_Err_Total[Channel_G] += CED_Err[Current_EQ_TableIndex][2];
			CED_Err_Total[Channel_G] +=
				(CED_Err[Current_EQ_TableIndex][3] & 0x7F) << 8;

			CED_Err_Total[Channel_R] += CED_Err[Current_EQ_TableIndex][4];
			CED_Err_Total[Channel_R] +=
				(CED_Err[Current_EQ_TableIndex][5] & 0x7F) << 8;

			EQ_DEBUG_PRINTF(
				"Channel B Current CED Err=%x, %x  Total Err=0x%x\n",
				(int)CED_Err[Current_EQ_TableIndex][1],
				(int)CED_Err[Current_EQ_TableIndex][0],
				(int)CED_Err_Total[Channel_B]);
			...
			if (iTE6805_DATA.CurrentPort != PORT0 ||
				...
				EQ_DEBUG_PRINTF(
					"Channel G Current CED Err=%x, %x  Total Err=0x%x\n",
					(int)CED_Err[Current_EQ_TableIndex][3],
					(int)CED_Err[Current_EQ_TableIndex][2],
					(int)CED_Err_Total[Channel_G]);
				EQ_DEBUG_PRINTF(
					"Channel R Current CED Err=%x, %x  Total Err=0x%x\n",
					(int)CED_Err[Current_EQ_TableIndex][5],
					(int)CED_Err[Current_EQ_TableIndex][4],
					(int)CED_Err_Total[Channel_R]);
...
void iTE6805_Get_DFE(iTE_u8 Channel_Color)
{
	...
	if (Flag_AMP_ALL_Timeout == TRUE) {
		EQ_DEBUG_PRINTF(
			"Flag_AMP_ALL_Timeout == TRUE , dont need to get DFE value \n");
		return;
	}
	...
	case Channel_B:
		...
		EQ_DEBUG_PRINTF(
			"!! Report Channel B DFE Value , EQ TargetRS = 0x%x!!\n",
			(int)TargetRS);
		break;
	case Channel_G:
		...
		EQ_DEBUG_PRINTF(
			"!! Report Channel G DFE Value , EQ TargetRS = 0x%x!!\n",
			(int)TargetRS);
		break;
	case Channel_R:
		...
		EQ_DEBUG_PRINTF(
			"!! Report Channel R DFE Value , EQ TargetRS = 0x%x!!\n",
			(int)TargetRS);
		break;
	default:
		break;
	...
	for (i = 0; i < sizeof(RS_Value); i++) {
		...
		/* EQ_DEBUG_PRINTF("Tap1_Sign=%d, Tap1=%x,  Tap2_Sign=%d  Tap2=%x,
		 * Tap3_Sign=%d, Tap3=%x\n", (int)Tap1_Sign, (int)Tap1_Value,
		 * (int)Tap2_Sign, (int)Tap2_Value, (int)Tap3_Sign, (int)Tap3_Value); */
		...
		if (Flag_Need_Adjust_RSValue == TRUE) {
			...
			if (iTE6805_DATA.ChipID == 0xA0 || iTE6805_DATA.ChipID == 0xA1) {
				...
				if (TimeOut_Now == 0 && ((Tap12_Value < MinTap12_Value) ||
										 (Tap12_Value == MinTap12_Value &&
										  Tap123_Value < MinTap123_Value))) {
	...
	/* after B0 because do not have RSSKEW EQ, so need to start manual DFE
	 * value or Manual EQ DFE setting will fake. */
#if (iTE68051 == TRUE || iTE68052 == TRUE)
	if (Flag_Need_Adjust_RSValue == TRUE ||
		(iTE6805_DATA.ChipID != 0xA0 && iTE6805_DATA.ChipID != 0xA1))
#else
...
void iTE6805_Set_DFE(iTE_u8 EQ_Value, iTE_u8 Type_Channel)
{
	...
	case Channel_G:
		...
		/* iTE6805 A0 if write value to REG 0x34F/0x74F, the Reg 4F will be
		 * also over write. */
#if (iTE68051 == TRUE || iTE68052 == TRUE)
		if (iTE6805_DATA.ChipID != 0xA0)
