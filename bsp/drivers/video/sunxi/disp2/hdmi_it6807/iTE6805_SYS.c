...
iTE_u8 iTE6805_CurrentStage = STAGE6805_INIT;
...
void iTE6805_MainBody(void)
{
	...
	iTE6805_Port_SetEDID_Body(iTE6805_DATA.US_SetEDID_Port,
							  iTE6805_DATA.US_SetEDID_Index);
#endif
	...
	if (iTE6805_DATA.STATEV != STATEV_Unplug &&
		iTE6805_DATA.STATEV != STATEV_VideoOff) {
...
void iTE6805_hdmirx_port0_SYS_irq(void)
{
	...
	if (Reg05h != 0x00) {
		...
		if (Reg05h & 0x40) {
			...
			HDMIRX_DEBUG_PRINT("# Port 0 Deskew Error #  deskewerrcnt=%d\n",
							   (int)deskewerrcnt);
#if (_ENABLE_AUTO_EQ_ == TRUE)
			if (deskewerrcnt > MAX_deskewerrcnt &&
				iTE6805_Check_HDMI2() == FALSE) {
				deskewerrcnt = 0;
		...
		if (Reg05h & 0x04) {
			...
			if (hdmirxrd(0x13) & 0x10) {
				...
				/* if((hdmirxrd(0x23) & BIT1) == 0) */ /* prevent customer
														  force 6805 not support
														  HDCP but this reset
														  will enable HDCP */
													   /* { */
	...
	if (Reg06h != 0x00) {
		...
		if ((Reg06h & 0x40) &&
			iTE6805_Check_PORT0_IS_MHL_Mode(PORT0) == MODE_HDMI) {
		...
		if (Reg06h & 0x10) {
			 ...
			 */
			/* try to clock valid , this case is 3.4Gup and not write SCDC to
			 * RX */
			if ((Current_Ch0_SymbolLockRst_Count >
				 Max_SymbolLockRst_Need_Rst_SCDC) &&
				(Current_Ch1_SymbolLockRst_Count >
				 Max_SymbolLockRst_Need_Rst_SCDC) &&
				(Current_Ch2_SymbolLockRst_Count >
				 Max_SymbolLockRst_Need_Rst_SCDC) &&
	...
	if (Reg09h != 0x00) {
		if (Reg09h & 0x01) {
...
#if (_ENABLE_6805_AS_HDCP_REPEATER_CODE_ == TRUE)
			if (iTE6805_DATA.ENABLE_6805_AS_HDCP_REPEATER == 1) {
				if (iTE6805_DATA.STATER_6805 ==
					STATER_6805_Wait_For_HDCP_Done) {
					iTE6805_DATA.Flag_HDCP_NOW = 1;
					iTE6805_DATA.CB_HDCP.iTE6805_Set_DownstreamHDCPVersion_CB(
						1); /* setting chip call back HDCP1 enabled */
			...
			if (iTE6805_Check_PORT0_IS_MHL_Mode(PORT0) &&
				Current_AuthStartCount++ > 2) {
				Current_AuthStartCount = 0;
				HDMIRX_DEBUG_PRINT(
					"# Port 0 HDCP Authentication Start Exceed , reset PATH_EN "
					"#\n");
				chgbank(0);
...
void iTE6805_hdmirx_port1_SYS_irq(void)
{
	...
	/* if (iTE6805_DATA.STATEV == STATEV_Unplug || iTE6805_DATA.STATEV ==
	 * STATEV_VideoOff || */
	...
	if (Reg0Ah != 0x00) {
		...
		if (Reg0Ah & 0x04) {
			...
			if (hdmirxrd(0x16) & 0x10) {
				...
				/* if((hdmirxrd(0x2B)&BIT1)== 0)	 @  prevent customer force */
				/* 6805 not support HDCP but this reset will enable HDCP */
				/* { */
	...
	if (Reg0Bh != 0x00) {
		...
		if ((Reg0Bh & 0x40) &&
			iTE6805_Check_PORT0_IS_MHL_Mode(PORT1) == MODE_HDMI) {
			HDMIRX_DEBUG_PRINT("# CH2 Symbol lock Rst #\n");
			Current_Ch2_SymbolLockRst_Count++;
		}
		...
		if (Reg0Bh & 0x01) {
			...
			if (hdmirxrd(0x17) & 0x38 && iTE6805_DATA.STATEEQ == STATEEQ_Off) {
				iTE6805_EQ_chg(STATEEQ_Off);
				iTE6805_EQ_chg(
					STATEEQ_ClockONDet_And_TriggerEQ); /* Check inside */
			}
#endif
...
void iTE6805_hdmirx_common_irq(void)
{
	...
	hdmirxwr(
		0x12,
		Reg12h & 0x5F); /* common  @  DRM check change to DRM_Detect function */
	...
	if (Reg11h != 0x00) {
		...
		if (Reg11h & BIT2) {
			...
			/* chgbank(2); */
			/* HDMI_VSIF_OUI = (hdmirxrd(0x2A) << 16) + (hdmirxrd(0x29) << 8) +
			 * (hdmirxrd(0x28)); */
			/* HDMI_VSIF_3DValid = (hdmirxrd(0x2C) & 0x01); */
			/* if (HDMI_VSIF_OUI == 0xC45DD8) HDMIRX_DEBUG_PRINT("Valid
			 * HDMI_VSIF ! \n"); */
			/* else if (HDMI_VSIF_OUI == 0x0C03) HDMIRX_DEBUG_PRINT("Valid
			 * HDMI1.4 VSIF !\n"); */
	...
	if (Reg12h != 0x00) {
		if (Reg12h & 0x80) {
			...
			HDMIRX_DEBUG_PRINT(
				"# VidParaChange_Sts=Reg1Bh=0x%02X Reg1Ah=0x%02X\n",
				(int)Reg1Bh, (int)Reg1Ah);

			hdmirxwr(0x12,
					 0x80); /* only parameter change need to clear INT here ,
							   or register 1A/1B can't be read after clear. */

			/* only downscale need this section */
			/* because parameter change need reset downscale setting when case
			 * 4096 change to 3840 but video still stable */

#if ((_MCU_8051_EVB_ == TRUE) || iTE6805_4K60_Mode == MODE_DownScale || \
	 iTE6805_4K60_Mode == MODE_EvenOdd_Plus_DownScale) /* DEMO */

			HActive = ((hdmirxrd(0x9E) & 0x3F) << 8) + hdmirxrd(0x9D);
			Reg1Ah =
				Reg1Ah & (BIT1 | BIT6 | BIT3); /* HDE Change/H Sync Width
												  Change/V Sync Width Change */
			...
			if (iTE6805_DATA.Flag_VidStable_Done == TRUE) {
				if (Flag_First_Time_VidStable_Done == TRUE) {
					...
					if (Flag_FirstTimeParameterChange == TRUE) {
						...
						if ((iTE6805_CurVTiming.HActive != 0 &&
							 (iTE6805_CurVTiming.HActive != HActive)) ||
							Reg1Ah || Reg1Bh) {
							HDMIRX_DEBUG_PRINT(
								"HActive = %d, before HActive = %d \n", HActive,
						...
						chgbank(0);
						iTE6805_CurVTiming.HActive =
							((hdmirxrd(0x9E) & 0x3F) << 8) + hdmirxrd(0x9D);
						iTE6805_CurVTiming.VActive =
							((hdmirxrd(0xA5) & 0x3F) << 8) + hdmirxrd(0xA4);
						if (iTE6805_Check_4K_Resolution() ||
							(iTE6805_CurVTiming.HActive >= 3000 &&
							 iTE6805_CurVTiming.VActive >= 2000)) {
							Flag_Trigger_DownScale_Reset = TRUE;
						}
#else
						if (iTE6805_Check_4K_Resolution()) {
					...
					VIDEOTIMNG_DEBUG_PRINTF(
						" Flag_Trigger_DownScale_Reset   = %2.2x\r\n",
						(int)Flag_Trigger_DownScale_Reset);

					if (Flag_Trigger_DownScale_Reset == TRUE) {
						HDMIRX_DEBUG_PRINT(
							" Set_DNScale in Parameter Change \n");

						iTE6805_Get_AVIInfoFrame_Info(); /* add here for
															setting DownScale
															variable */
						iTE6805_Get_VID_Info(); /* this need time to calculate,
												   put this here for save time
												   and get item for setting
												   downscale */
						...
						Flag_Disable_NewAVIInfoFrame_When_Parameter_Change =
							TRUE; /* for parameter change mean AVI infoframe
									 already setting here */
		...
		if (Reg12h & 0x01) {
			...
			chgbank(2);
			AVIDB = hdmirxrd(
				REG_RX_AVI_DB1); /* fix to only DB1 (color space) need to
									compare, checksum don't needed anymore. */
			chgbank(0);

			/* some device may send different resolution  but video still
			 * stable, but it will send new AVI InfoFrame to tell you */
			/* so need judge New AVI InfoFrame */
			if (iTE6805_DATA.Flag_VidStable_Done != TRUE) {
				HDMIRX_DEBUG_PRINT(
					"Flag_VidStable_Done!=TRUE, Save AVI Infoframe to "
					"variable. \n");
				...
				if (AVIDB != prevAVIDB) {
					prevAVIDB = AVIDB;
					if (Flag_Disable_NewAVIInfoFrame_When_Parameter_Change ==
						TRUE) {
						/* diable Flag_NewAVIInfoFrame because video parameter
						 * change already setting on top, do not trigger this
						 * Flag for STATEV_VidStable in vid_fsm */
						Flag_Disable_NewAVIInfoFrame_When_Parameter_Change =
							FALSE;
	...
	if (Reg1Dh != 0x00) {
		if (Reg1Dh & 0x01) {
			...
			if (iTE6805_DATA.ENABLE_6805_AS_HDCP_REPEATER == 1) {
				if (iTE6805_DATA.STATER_6805 ==
					...
					iTE6805_DATA.CB_HDCP.iTE6805_Set_DownstreamHDCPVersion_CB(
						2); /* setting chip call back HDCP2 enabled */
					iTE6805_HDCP_Repeater_chg(STATER_6805_HDCP_AuthPart2_Start);
				}
			}
#endif
		...
		if (Reg1Dh & 0x08) {
			...
			if ((iTE6805_Check_HDMI2() == FALSE) &&
				...
				/* when hdmi1.x 30m/10cm (10cm with EQ 0x01 will check ok but
				 * EQ still fail, so need auto EQ again when EQ1.x and only
				 * trigger once) */
				if (iTE6805_BitErr_Check_Again() == FALSE) {
					EQ_DEBUG_PRINTF(
						"******EQ Done But Find CED Error when Encryption "
						"change, Restart Manual EQ ******\n");
					iTE6805_EQ_chg(STATEEQ_ClockONDet_And_TriggerEQ);
				}
			}
#endif
...
void iTE6805_vid_fsm(void)
{
	...
	case STATEV_VidStable:
		...
		if (Flag_NewAVIInfoFrame == TRUE) {
			HDMIRX_DEBUG_PRINT(
				"!!! New AVI InfoFrmae in STATEV_VidStable !!!\n");
			iTE6805_Get_AVIInfoFrame_Info(); /* add here for setting DownScale
												variable */
			iTE6805_Check_ColorChange_Update();
			iTE6805_Get_VID_Info(); /* need this for downscale setting
									   parameter */
			iTE6805_Show_VID_Info();

#if (iTE68051 == TRUE)
			iTE68051_Video_Output_Setting(); /* DaulPixel setting/ DownScaling
												setting/ Demo Setting/ PIN
												Config Setting only for 6805A0
											 */
#endif
			...
			iTE6805_Set_1B0_By_PixelClock(); /* 20180605 modify single output
												set to 0, 0713 Andrew suggest if
												PCLK < 25 need to set 1B0[0] to
												0, else 1 */
			iTE6805_Enable_Video_Output();
			iTE6805_Reset_Video_Logic(); /* can't put reset video logic here or
											vidstable will be keep reset reset
											reset reset .... 0330 */

			Flag_NewAVIInfoFrame = FALSE;
			Flag_FirstTimeParameterChange = TRUE;
#if (iTE68051 == TRUE && _MCU_8051_EVB_ == TRUE)
			/* If new avi info frame arrive , need init LVDS again, 15m line
			 * might had new AVI infoframe condition */
			iTE6805_Init_6028LVDS(0); /* init first 6028 chip */
			iTE6805_Init_6028LVDS(1); /* init first 6028 chip */
#endif
			iTE6805_Set_Video_Tristate(
				TRISTATE_OFF); /* tristate must put after setting single/dual
								  pixel mode, or can't judge it */
		}

		/* If setting AV by last stable video, and do not receive av mute clear
		 * pkt */
		...
		if (!(hdmirxrd(0x81) & BIT6)) {
			...
			/* need to keep update channel status prevent from 48K to 44K and
			 * somehow not trigger FIFO error and status not update */
			/* need to value for setting other chip audio setting */
			iTE6805_DATA.Audio_Frequency =
				((hdmirxrd(0xB5) & 0xC0) >> 2) + (hdmirxrd(0xB5) & 0x0F);
		}
	...
	case STATEA_AudioOn:
		if (Current_AudioMonitor_Count-- == 0) {
			...
			if (Flag_FirstTimeAudioMonitor == TRUE) {
				prevAudioB0_Status =
					hdmirxrd(0xB0) & 0xF0;			 /* compare mask 0xF0 */
				prevAudioB1_Status = hdmirxrd(0xB1); /* compare mask 0xFF */
				prevAudioB2_CHStatus =
					hdmirxrd(0xB2) & BIT1; /* compare mask BIT1 */
				Flag_FirstTimeAudioMonitor = FALSE;
				if (prevAudioB2_CHStatus == 0x00) {
					/* If it is not LPCM i.e. compress audio , disable auto
					 * mute. */
...
void iTE6805_vid_chg(STATEV_Type NewState)
{
	...
	case STATEV_VideoOff:
		...
		iTE6805_Set_HPD_Ctrl(
			PORT1, HPD_LOW); /* if downstream switch off the state, it should
								be hold and HPD goes to off. */
		break;
	case STATEV_Unplug:
		...
		if (iTE6805_DATA.ChipID == 0xA0 || iTE6805_DATA.ChipID == 0xA1) {
			...
			chgbank(0);
			hdmirxset(0xC5, BIT4,
					  BIT4); /* Unplug need to reset EDID, because EDID reset
								is also reset SCDC in hardware, or SCDC may hang
								in sometimes */
			hdmirxset(0xC5, BIT4,
					  0x00); /* Unplug need to reset EDID, because EDID reset
								is also reset SCDC in hardware, or SCDC may hang
								in sometimes */
		...
		hdmirxset(0x23, 0xFD,
				  0xAD); /* fix to AD for AVMute Issue, MHL->HDMI pattern gen
							will trigger keep AVMute on */

#if (_ENABLE_IT6805_CEC_ == TRUE && _ENABLE_IT6805_MHL_FUNCTION_ == TRUE)
		iTE6805_CEC_INIT(); /* fix to AD for AVMute Issue, MHL->HDMI pattern
							   gen will trigger keep AVMute on */
#endif
		...
		if (iTE6805_DATA.ENABLE_6805_AS_HDCP_REPEATER == 1) {
			if (iTE6805_DATA.STATER_6805 == STATER_6805_Wait_For_HDCP_Done) {
				...
				iTE6805_DATA.CB_HDCP.iTE6805_Set_DownstreamHDCPVersion_CB(
					0); /* setting chip call back HDCP2 enabled */
			}
		}
#endif
	...
	case STATEV_VidStable:
...
#if (iTE68051 == TRUE)
		iTE68051_Video_Output_Setting(); /* DaulPixel setting/ DownScaling
											setting/ Demo Setting/ PIN Config
											Setting */
#endif

#if (iTE68052 == TRUE)
		iTE68052_Video_Output_Setting(); /* DaulPixel setting/ DownScaling
											setting/ Demo Setting/ PIN Config
											Setting */
#endif

#if (iTE6807 == TRUE)
		iTE6807_Video_Output_Setting(); /* DaulPixel setting/ DownScaling
										   setting/ Demo Setting/ PIN Config
										   Setting */
#endif

		iTE6805_Set_1B0_By_PixelClock(); /* 20180605 modify single output set
											to 0, 0713 Andrew suggest if PCLK <
											25 need to set 1B0[0] to 0, else 1
										 */

		iTE6805_Enable_Video_Output(); /* AVI Info Frame Handler , CSC Table
										  Handler */

		/* need to set to AVMUTE_ON for might lost the PKT in the moment
		 * between Source-Video-In and Sink-Video-Stable/ */
		/* and detect AVMUTE off status in the vid_fsm selection */
		iTE6805_Set_AVMute(AVMUTE_ON);

		iTE6805_aud_chg(STATEA_RequestAudio);

		/* 0330 fix it : no warning, because video logic reset need to reset
		 * parameter change INT and SCDT INT or it will be SCDT on->off */
		/* this cammand is wrong-> !!!!!! WARNING !!!!!!can't put logic reset
		 * between iTE6805_Set_AVMute and iTE6805_Set_AVMute or video tristate
		 * off then there will a SCDT off->SCDT on->SCDT off->...... */
		iTE6805_Reset_Video_Logic();

		iTE6805_Set_Video_Tristate(
			TRISTATE_OFF); /* tristate must put after setting single/dual pixel
							  mode, or can't judge it */

#if (iTE68051 == TRUE && _MCU_8051_EVB_ == TRUE)
		delay1ms(100);
		/* 6028 init move from beginning to here because if init in the
		 * beginning, the DE between 6028 and 6805 can't sync */
		/* and the rightest V-line in the screen will be all block or white,
		 * need move 6028 init from beginning to here. */
		iTE6805_Init_6028LVDS(0); /* init first 6028 chip */
		iTE6805_Init_6028LVDS(1); /* init first 6028 chip */
		...
		/* if this state here mean ECC error before, so if vid stable again,
		 * release EQ to EQDone state. */
		/* release state STATEEQ_KeepEQStateUntil5VOff to normal state */
#if (_ENABLE_AUTO_EQ_ == TRUE)
		if (iTE6805_DATA.STATEEQ == STATEEQ_KeepEQStateUntil5VOff) {
			HDMIRX_DEBUG_PRINT(
				"Release state STATEEQ_KeepEQStateUntil5VOff to normal "
				"state(STATEEQ_EQDone)!!!\n");
...
#if Debug_message
		iTE6805_Show_VID_Info(); /* move to here for saving time to tristate
									off */
#endif
...
void iTE6805_aud_chg(STATEA_Type NewState)
{
	...
	case STATEA_RequestAudio:
		...
		chgbank(0);
		hdmirxset(0x81, BIT6,
				  0x00); /* RegForce_FS : 0: Disable Force Audio FS mode */
	...
	case STATEA_WaitForReady:
		...
		hdmirxset(0x8C, BIT4,
				  0x00); /* clear RegHWMuteClr for clear H/W Mute */
		Current_AudioCheck_Count = TIMEOUT_AUDIO_CHECK_COUNT;
		break;
	case STATEA_AudioOn:
...
void iTE6805_Port_SetEDID_Body(iTE_u8 SET_PORT, iTE_u8 EDID_INDEX)
{
	...
	if (iTE6805_DATA.US_EDID_INDEX[SET_PORT] == EDID_INDEX) {
		HDMIRX_DEBUG_PRINT(
			"iTE6805_Port_SetEDID called, EDID Same so do nothing! \n");
		return;
	}
...
void iTE6805_Port_Select_Body(iTE_u8 ucPortSel)
{
	...
	if (ucPortSel == PORT0) {
		...
		iTE6805_Set_HPD_Ctrl(
			PORT1, HPD_LOW); /* non main port need to set HPD low, or sometimes
								tx can't resend output when HPD no toggle */
#if (_ENABLE_IT6805_MHL_FUNCTION_ == TRUE)
		if (iTE6805_Check_PORT0_IS_MHL_Mode(PORT0)) {
...
#endif
		iTE6805_Set_HPD_Ctrl(
			PORT0, HPD_LOW); /* non main port need to set HPD low, or sometimes
								tx can't resend output when HPD no toggle */
	}
	chgbank(0);
	...
	if (iTE6805_DATA.US_EDID_INDEX[PORT0] !=
		iTE6805_DATA.US_EDID_INDEX[PORT1]) {
		iTE6805_EDID_RAMInitial(); /* add for changed edid when PORT select */
	}
#endif

#if defined(_ENABLE_6805_INT_MODE_FUNCTION_) && \
	(_ENABLE_6805_INT_MODE_FUNCTION_ == TRUE)
	...
	if (iTE6805_DATA.CurrentPort == PORT0) {
		...
		hdmirxset(
			0xC5, BIT4,
			BIT4); /* Unplug need to reset EDID, because EDID reset is also
					  reset SCDC in hardware, or SCDC may hang in sometimes */
		delay1ms(1);
		hdmirxset(
			0xC5, BIT4,
			0x00); /* Unplug need to reset EDID, because EDID reset is also
					  reset SCDC in hardware, or SCDC may hang in sometimes */
	} else {
		/* hdmirxwr(0x25, 0xDF); */
		hdmirxwr(0x26, 0xFF);
...
void iTE6805_Port_Reset_Body(iTE_u8 ucPortSel)
{
	...
	if (ucPortSel != 0 && ucPortSel != 1) {
		...
		HDMIRX_DEBUG_PRINT(
			"\n\nCustomer PortReset need to setting Current Port to 0/1, can't "
			"be other value ! \n\n");
		return;
	}
...
void iTE68051_Video_Output_Setting(void)
{
	...
	if (iTE6805_DATA._iTE6805_4K_Mode_ == MODE_EvenOdd ||
		...
		if (iTE6805_Check_4K_Resolution() ||
			iTE6805_CurVTiming.PCLK > DUAL_PIXEL_MODE_PCLK_CONDITION) {
			...
			iTE6805_DATA.Flag_Pixel_Mode = FALSE;
			iTE6805_DATA._iTE6805_4K_Mode_ =
				MODE_EvenOdd; /* not 4k default is even odd mode */
		}
		...
		hdmirxset(0x64, BIT1, BIT1); /* setting _iTE68051_4K_Mode_ hardware
										suggest reset REG64[1] */
		hdmirxset(0x64, BIT1, 0);
	}
	...
	if (iTE6805_DATA._iTE6805_4K_Mode_ == MODE_DownScale ||
		...
		iTE6805_CurVTiming.VActive =
			((hdmirxrd(0xA5) & 0x3F) << 8) + hdmirxrd(0xA4);
		if ((iTE6805_Check_4K_Resolution() ||
			 ((iTE6805_CurVTiming.HActive << iTE6805_DATA.Flag_IS_YUV420) >=
				  3000 &&
			  iTE6805_CurVTiming.VActive >= 2000)) &&
			iTE6805_Check_Support4KTiming()) {
		...
		if (iTE6805_CurVTiming.PCLK > DUAL_PIXEL_MODE_PCLK_CONDITION &&
			iTE6805_DATA.Flag_DownScale == FALSE) {
			HDMIRX_DEBUG_PRINT(
				"!!!!!! Set Dual Pixel Mode by DUAL_PIXEL_MODE_PCLK_CONDITION "
				"!!!!!!\n");
			iTE6805_Set_LVDS_Video_Path(2); /* 2 lane for dual pixel mode */
			iTE6805_DATA.Flag_Pixel_Mode = TRUE;
		} else {
...
void iTE68052_Video_Output_Setting(void)
{
	...
	chgbank(0);
	hdmirxset(0x36, BIT1,
			  0x00); /* 0928 add for 68021 need to set this bit to 0 or some
						chip HDMI RX will stable but target LVDS RX keep
						unstable, 68051 need to set to 1, and default is 1 */
					 /* set Dual Pixel output mode to Odd/Even Mode */
	hdmirxset(0x64, BIT2, 0);
	/* setting LREnable need to reset REG64[1] */
	hdmirxset(0x64, BIT1, BIT1);
	...
	hdmirxwr(0xC1, 0x23);
	hdmirxwr(0xC2, 0x0B);
	hdmirxwr(0xCB, 0x0F); /* 20190905 Min-Hui, YauTe suggest this value need
							 change to 0x0F */
	hdmirxset(0x20, 0x40, 0x40); /* reset Downscale !!!!! */
	chgbank(1);
	hdmirxset(
		0xC0, 0x04,
		0x04); /* setting to single pixel mode and Enable LVDS output module */
	...
	if (iTE6805_DATA._iTE6805_4K_Mode_ == MODE_EvenOdd ||
		...
		if (iTE6805_Check_4K_Resolution() ||
			iTE6805_CurVTiming.PCLK > DUAL_PIXEL_MODE_PCLK_CONDITION) {
			HDMIRX_DEBUG_PRINT("!!!!!! Set Dual Pixel Mode !!!!!!\n");
			iTE6805_Set_LVDS_Video_Path(
				4); /* 4 lane for dual pixel mode when 68052 */
			iTE6805_DATA.Flag_Pixel_Mode = TRUE;
		} else {
			HDMIRX_DEBUG_PRINT("!!!!!! Set Single Pixel Mode !!!!!!\n");
		...
		hdmirxset(0x64, BIT1, BIT1); /* setting _iTE6805_4K_Mode_ hardware
										suggest reset REG64[1] */
		hdmirxset(0x64, BIT1, 0);
	}
...
void iTE6807_Video_Output_Setting(void)
{
	...
	} else if (iTE6805_Check_4K_Resolution() ||
			   iTE6805_CurVTiming.PCLK > 320000) {
		iTE6805_DATA.VBO_LaneCount = 3;
		HDMIRX_DEBUG_PRINT(
			"!!!!!! Set VBO_LaneCount = 8 Lan (variable "
			"iTE6805_DATA.VBO_LaneCount = 3)!!!!!!\n");
	} else if (iTE6805_CurVTiming.PCLK > 160000) {
		iTE6805_DATA.VBO_LaneCount = 2;
		HDMIRX_DEBUG_PRINT(
			"!!!!!! Set VBO_LaneCount = 4 Lan (variable "
			"iTE6805_DATA.VBO_LaneCount = 2)!!!!!!\n");
	} else if (iTE6805_CurVTiming.PCLK > 100000) {
		iTE6805_DATA.VBO_LaneCount = 1;
		HDMIRX_DEBUG_PRINT(
			"!!!!!! Set VBO_LaneCount = 2 Lan (variable "
			"iTE6805_DATA.VBO_LaneCount = 1)!!!!!!\n");
	} else {
		iTE6805_DATA.VBO_LaneCount = 0;
		HDMIRX_DEBUG_PRINT(
			"!!!!!! Set VBO_LaneCount = 1 Lan (variable "
			"iTE6805_DATA.VBO_LaneCount = 0)!!!!!!\n");
	}
	...
	case 0x40:
		...
		VIDEOTIMNG_DEBUG_PRINTF(
			"8 b, set ByteMode = 3 (variable iTE6805_DATA.VBO_ByteMode = 0)\n");
		break;
	case 0x50:
		iTE6805_DATA.VBO_ByteMode = 1;
		VIDEOTIMNG_DEBUG_PRINTF(
			"10 b, set ByteMode = 4 (variable iTE6805_DATA.VBO_ByteMode = "
			"1)\n");
		break;
	case 0x60:
		iTE6805_DATA.VBO_ByteMode = 2;
		VIDEOTIMNG_DEBUG_PRINTF(
			"12 b, set ByteMode = 5 (variable iTE6805_DATA.VBO_ByteMode = "
			"2)\n");
		break;
	default:
		iTE6805_DATA.VBO_ByteMode = 0;
	...
	iTE6805_DATA.VBO_ByteMode = 0;
	VIDEOTIMNG_DEBUG_PRINTF(
		"Force ByteMode = 3B (variable iTE6805_DATA.VBO_ByteMode = 0)\n");
#elif (Force_ByteMode == 4)
	iTE6805_DATA.VBO_ByteMode = 1;
	VIDEOTIMNG_DEBUG_PRINTF(
		"Force ByteMode = 4B (variable iTE6805_DATA.VBO_ByteMode = 1)\n");
#elif (Force_ByteMode == 5)
	iTE6805_DATA.VBO_ByteMode = 2;
	VIDEOTIMNG_DEBUG_PRINTF(
		"Force ByteMode = 5B (variable iTE6805_DATA.VBO_ByteMode = 2)\n");
#endif
#endif
	...
	hdmirxset(
		0xC0, 0xF0,
		(iTE6805_DATA.VBO_LaneCount << 4) + (iTE6805_DATA.VBO_ByteMode << 6));
	...
	if (VBOCLKSpd == 3) {
		hdmirxset(0xC6, 0x07,
				  0x04); /* 20180822 emily , don't use autoXPHSDet */
	} else {
		hdmirxset(0xC6, 0x07,
				  0x03); /* 20180822 emily , don't use autoXPHSDet */
	}
	...
	/* HDMIRX_DEBUG_PRINT("VBOTX_IP_Status=%2X , VBOTX_XP_Status=%2X,
	 * VBOTX_DRVPWD=%2X\n",(int) Reg_IP_Status,(int) Reg_XP_Status,(int)
	 * Reg_DRVPWD); */
...
void iTE6805_Enable_Video_Output(void)
{
	...
	/* REG6B[5:4]: Reg_ColMod_Set Input color mode set 00: RGB mode - 01:
	 * YUV422 mode, 10: YUV444 mode, 11: YUV420 mode */
	chgbank(0);
	if (iTE6805_Check_HDMI_OR_DVI_Mode(iTE6805_DATA.CurrentPort) == MODE_HDMI) {
		...
		hdmirxset(0x6B, 0x30,
				  iTE6805_DATA.AVIInfoFrame_Input_ColorFormat
					  << 4); /* seting input format by info frame ??? do not
								need ??? */
		hdmirxset(0x6B, 0x01,
				  0x00); /* 0: Input color mode auto detect , 1: Force input
							color mode as bit[3:2] setting */
	} else {
		iTE6805_DATA.Flag_InputMode = MODE_DVI;
		HDMIRX_DEBUG_PRINT("---- CSC DVI mode ----\n");
		hdmirxset(0x6B, 0x30, 0x10); /* seting input format to RGB */
		hdmirxset(0x6B, 0x01,
				  0x01); /* 0: Input color mode auto detect , 1: Force input
							color mode as bit[3:2] setting */
	...
	if (iTE6805_DATA.US_Flag_PYPASS_CSC == TRUE) {
		/* force ouput color = input color , equal to bypass csc , do not
		 * powerdown CSC because powerdown CSC also powerdown downscale */
		iTE6805_DATA.US_Output_ColorFormat =
			iTE6805_DATA.AVIInfoFrame_Input_ColorFormat;

#if (ENABLE_YUV420_CONVERT_TO_RGB_WHEN_CSC_BYPASS == TRUE)
		if (iTE6805_DATA.AVIInfoFrame_Input_ColorFormat ==
			Color_Format_YUV420) {
			iTE6805_DATA.US_Output_ColorFormat = Color_Format_RGB;
			HDMIRX_DEBUG_PRINT(
				"---- CSC BYPASS mode and Input = YUV420, need to force to "
				"RGB444 ----\n");
		}
#endif
	...
	chgbank(0);
	hdmirxwr(0x6E, 0xE0); /* default bit4 = 0 = setting to full range, 20190403
							 fix to set 0xE0 or 12b low bit may cut off. */
	chgbank(1);
	hdmirxwr(0x86, 0x00);
	chgbank(0);
	...
	if ((iTE6805_DATA.US_Output_ColorFormat == Color_Format_RGB) &&
		...
		hdmirxset(
			0x6C, 0x03,
			0x03); /* CSC Sel 00: bypass CSC 10: RGB to YUV 11: YUV to RGB */
...
#endif
		{
			if (iTE6805_DATA.AVIInfoFrame_Colorimetry == Colormetry_ITU709) {
				CSCMtx_Type = CSCMtx_YUV2RGB_ITU709_00_255; /* when 709 format
															   always to */
				/* RGB full range */
			} else if (iTE6805_DATA.AVIInfoFrame_Colorimetry ==
						   Colormetry_Extend &&
					   (iTE6805_DATA.AVIInfoFrame_ExtendedColorimetry == 0x05 ||
						iTE6805_DATA.AVIInfoFrame_ExtendedColorimetry ==
							0x06)) {
				/* Extend Colorimetry = BT2020  						 @  Extend
				 * Colorimetry = */
				/* BT2020 */
				/* this Matrix is BT2020 YUV to BT2020 RGB, not normal */
				/* limit/full range RGB */
				CSCMtx_Type =
					CSCMtx_YUV2RGB_BT2020_00_255; /* for BT.2020 CSC */
			} else {
				/* Colormetry_ITU601 */
				CSCMtx_Type = CSCMtx_YUV2RGB_ITU601_00_255;
			}
		}
	} else if (iTE6805_DATA.AVIInfoFrame_Input_ColorFormat ==
				   Color_Format_RGB &&
		...
		hdmirxset(
			0x6C, 0x03,
			0x02); /* CSC Sel 00: bypass CSC 10: RGB to YUV 11: YUV to RGB */
		if (iTE6805_DATA.AVIInfoFrame_Colorimetry == Colormetry_ITU709) {
			if (iTE6805_DATA.AVIInfoFrame_RGBQuantizationRange ==
				RGB_RANGE_LIMIT) {
		...
		hdmirxset(
			0x6C, 0x03,
			0x00); /* CSC Sel 00: bypass CSC 10: RGB to YUV 11: YUV to RGB */

#if ENABLE_RGB_LIMIT_TO_RGB_FULL_RANGE

		/* do not need RGB limit to FULL range anymore, this gray color stage
		 * may not expect */
		if (iTE6805_DATA.AVIInfoFrame_Input_ColorFormat == Color_Format_RGB) {
			if (iTE6805_DATA.AVIInfoFrame_RGBQuantizationRange ==
				...
				HDMIRX_DEBUG_PRINT(
					"using CSCMtx_RGB_16_235_RGB_00_255\r\n\r\n\r\n");
				CSCMtx_Type = CSCMtx_RGB_16_235_RGB_00_255;
				hdmirxset(0x6E, 0x80,
						  0x00); /* need to disable hardware auto csc select or
									hardware will auto set to BYPASS */
				hdmirxset(0x6C, 0x03,
						  0x02); /* CSC Sel 00: bypass CSC 10: RGB to YUV 11:
									YUV to RGB ??? come from 6802 */
				chgbank(1);
				hdmirxwr(0x86,
						 0x0F); /* more setting only for RGB Full Range */
				chgbank(0);
			}
		}
#endif
	...
	if (CSCMtx_Type != CSCMtx_Unknown) {
		...
		chgbank(1);
		hdmirxbwr(0x70, sizeof(CSC_Matrix[0]),
				  (iTE_u8 *)&CSC_Matrix[CSCMtx_Type][0]);
		chgbank(0);
	} else {
		/* mark it lool the command blow */
		/* hdmirxset(0x4F,BIT6,BIT6);  @ 1: power down color space conversion
		 * logic */
		chgbank(1);
	...
	/* follow 6802 (Input mode is YUV444/Input mode is YUV420 and Output mode
	 * is YUV444) or Input/Output = RGB */
	/* 20170119 can't power down CSC, */
	/* if power down CSC, Dualpixel mode LR mode/OddEven Mode will be shut down
	 * because CSC shut down */
...
void iTE6805_Enable_Audio_Output(void)
{
	...
	/* RegForce_CTSMode : need to set to 1 for get the CTS in the PKT, 0
	 * repersent nothing (HW using) */
	/* so set to 1 to get CTS in the REG2C1/2C2/2C0 */
	hdmirxset(0x86, BIT0, BIT0);
	chgbank(2);
	iTE6805_CurVTiming.N = ((iTE_u32)hdmirxrd(0xBE) << 12) +
						   ((iTE_u32)hdmirxrd(0xBF) << 4) +
						   ((iTE_u32)hdmirxrd(0xC0) & 0x0F);
	...
	Audio_CH_Status =
		((hdmirxrd(0xB5) & 0xC0) >> 2) +
		(hdmirxrd(0xB5) &
		 0x0F); /* Audio_CH_Status : Audio Channel status decoder value[31:24]
	...
	/* SW caulate Sampling Frequency equal to 60958-3 SPEC channel status, no
	 * need to enable Force FS mode */
	if (Audio_CH_Status == iTE6805_DATA.Force_Sampling_Frequency) {
		HDMIRX_AUDIO_PRINTF(
			"Audio_CH_Status == iTE6805_DATA.Force_Sampling_Frequency reset "
			"Audio \r\n");
		if (hdmirxrd(0x81) & BIT6)
			resetAudio =
				1; /* If Already Force FS Mode, Need to reset Audio; */
	...
	if (Current_AudioSamplingFreq_ErrorCount >
		Max_AudioSamplingFreq_ErrorCount) {
		hdmirxset(0x81, BIT6, BIT6); /* RegForce_FS : Force Audio FS mode */
		hdmirxset(0x8A, 0x3F,
				  iTE6805_DATA
					  .Force_Sampling_Frequency); /* RegFS_Set[5:0] : Software
...
#if (_ENABLE_6805_AS_HDCP_REPEATER_CODE_ == TRUE)
		if (iTE6805_DATA.Flag_EDIDReady == 0 &&
...
#if (ENABLE_6805_POWER_SAVING_MODE == TRUE)
		if (iTE6805_Check_PORT0_IS_MHL_Mode(PORT0) == MODE_HDMI &&
...
void iTE6805_INT_ECC_ERROR(void)
{
	if ((Current_ECCError_Count++) > TIMEOUT_ECC_ERROR) {
		...
		if (Current_ECCAbnormal_Count == 3) {
			...
			/* force release to EQ off ! because lock by
			 * STATEEQ_KeepEQStateUntil5VOff */
			iTE6805_DEBUG_INT_PRINTF(
				"++++++++++++++ECC ERROR TIMEOUT ++++++++++++++ Force EQ "
				"STATEEQ_Off \n");
			iTE6805_DATA.STATEEQ = STATEEQ_Off;
			iTE6805_EQ_chg(STATEEQ_Off);
		...
		iTE6805_DEBUG_INT_PRINTF(
			"++++++++++++++ECC ERROR TIMEOUT ++++++++++++++ Force EQ "
			"STATEEQ_KeepEQStateUntil5VOff \n");
		/* Force EQ do not change anymore, only using iTE6805_DATA.STATEEQ =
		 * STATEEQ_KeepEQStateUntil5VOff here */
...
iTE_u8 iTE6805_Identify_Chip(void)
{
	...
	vendor_id[2] = REG02;
	vendor_id[3] = REG03;
	if (REG00 != 0x54 || REG01 != 0x49 || (!(REG02 == 0x05 || REG02 == 0x07)) ||
		REG03 != 0x68) {
...
void iTE6805_DRM_Detect(void)
{
	...
	/* If check 5 times Reg11h[6] = 0 , then that's really have DRM PKT, or it
	 * not */
	...
	if (iTE6805_DATA.Flag_HAVE_DRM_PKT) {
		...
		if ((Reg12h & 0x20) || DRM_StatusChange) {
			...
			for (i = 0; i < 28; i++) {
				...
				iTE6805_DEBUG_INT_PRINTF(" 0x%02X",
										 (int)iTE6805_DATA.DRM_DB[i]);
			}
			iTE6805_DEBUG_INT_PRINTF("\n\n");
...
void iTE6805_CHECK_DRMInfoframe(iTE_u8 *pFlag_HAVE_DRM_PKT,
								iTE_u8 *pFlag_NEW_DRM_PKT_RECEIVE)
{
...
void iTE6805_GET_Input_Type(iTE_u8 *pFlag_Input_Type)
{
	...
	*pFlag_Input_Type = iTE6805_DATA.Flag_InputMode;
	...
	if(iTE6805_DATA.CurrentPort == PORT0)
	{
		...
		{
			if(iTE6805_Check_HDMI_OR_DVI_Mode(iTE6805_DATA.CurrentPort) ==
	MODE_HDMI)
			{
...
{
	/* If Below parameter need to send to customer upper layer, need to x = x*2
	 * when YUV420 */
...
void iTE6805_GET_EQ_Result(iTE_u8 *pEQ_B_Channel, iTE_u8 *pEQ_G_Channel,
						   iTE_u8 *pEQ_R_Channel)
{
...
iTE_u8 iTE6805_GET_Input_Is_LimitRange(void)
{
	if (iTE6805_DATA.Flag_InputMode == MODE_HDMI) {
		if (iTE6805_DATA.AVIInfoFrame_Input_ColorFormat ==
			...
			iTE6805_DATA.AVIInfoFrame_Input_ColorFormat ==
				Color_Format_YUV420) {
			if (iTE6805_DATA.AVIInfoFrame_YUVQuantizationRange ==
				YUV_RANGE_FULL) {
				return FALSE;
			} else {
				return TRUE;
...
void iTE6805_Detect3DFrame(void)
{
	...
	/* if ((s_Current3DFr.PB1 == 0x03) && (s_Current3DFr.PB2 == 0x0C) &&
	 * (s_Current3DFr.PB3 == 0x00)) { */
...
void iTE6805_Check_ColorChange_Update(void)
{
	...
	if (bAVIInfoFrame_Input_ColorFormat !=
		iTE6805_DATA.AVIInfoFrame_Input_ColorFormat) {
	...
	if (bAVIInfoFrame_ExtendedColorimetry !=
		iTE6805_DATA.AVIInfoFrame_ExtendedColorimetry) {
		iTE6805_DATA.Flag_AVI_ColorChange |= 0x04;
	}
	if (bAVIInfoFrame_RGBQuantizationRange !=
		iTE6805_DATA.AVIInfoFrame_RGBQuantizationRange) {
		iTE6805_DATA.Flag_AVI_ColorChange |= 0x08;
	}
	if (bAVIInfoFrame_YUVQuantizationRange !=
		iTE6805_DATA.AVIInfoFrame_YUVQuantizationRange) {
	...
	if (iTE6805_DATA.Flag_AVI_ColorChange != 0x00) {
		...
		bAVIInfoFrame_Input_ColorFormat =
			iTE6805_DATA.AVIInfoFrame_Input_ColorFormat;
		bAVIInfoFrame_Colorimetry = iTE6805_DATA.AVIInfoFrame_Colorimetry;
		bAVIInfoFrame_RGBQuantizationRange =
			iTE6805_DATA.AVIInfoFrame_RGBQuantizationRange;
		bAVIInfoFrame_YUVQuantizationRange =
			iTE6805_DATA.AVIInfoFrame_YUVQuantizationRange;
		bAVIInfoFrame_ExtendedColorimetry =
			iTE6805_DATA.AVIInfoFrame_ExtendedColorimetry;
	}
}
...
void iTE6805_Set_EQ_LEVEL_Body(iTE_u8 PORT, EQ_LEVEL EQ)
{
	...
	if (iTE6805_DATA.EQ_Customer_Setting[PORT] != EQ_AUTO) {
		EQ_DEBUG_PRINTF(
			"******EQ change to STATEEQ_KeepEQStateUntil5VOff state because "
			"Customer not set to EQ_AUTO ******\n");
		iTE6805_DATA.STATEEQ =
			STATEEQ_KeepEQStateUntil5VOff; /* keep it until EQ Done */
	}
