...
#define WAITABORTNUM \
	(3000) /* (300)			/* change from 3000 to 5000 for ATC 6.3.6.5 */
...
#define T10usSrcSel \
	TRUE /* FALSE: 100ms calibration , TRUR: 27MHz Crystal(only IT6802) */
#define CDDGSel 3
...
#define ADOPTER_ID_H                                         \
	0x02 /* !!!!!!!!!!!!!!!!!!!! need to using IT ADOPTER ID \
			!!!!!!!!!!!!!!!!!!!! */
#define ADOPTER_ID_L                                         \
	0x45 /* !!!!!!!!!!!!!!!!!!!! need to using IT ADOPTER ID \
			!!!!!!!!!!!!!!!!!!!! */
...
#define SUPP_VID_LINK                                             \
	((SUPP_VGA << 5) | (SUPP_ISLANDS << 4) | (SUPP_PPIXEL << 3) | \
	 (SUPP_YCBCR422 << 2) | (SUPP_YCBCR444 << 1) | (SUPP_RGB444))
...
#define VIDEO_TYPE                                                          \
	((SUPP_VT << 7) | (VT_GAME << 3) | (VT_CINEMA << 2) | (VT_PHOTO << 1) | \
	 (VT_GRAPHICS))
...
#define LOG_DEV_MAP                                                          \
	((LD_GUI << 7) | (LD_SPEAKE << 6) | (LD_RECORD << 5) | (LD_TUNER << 4) | \
		 (LD_MEDIA << 3) ||                                                  \
	 (LD_AUDIO << 2) | (LD_VIDEO << 1) | (LD_DISPLAY))
...
#define FEATURE_FLAG                                                   \
	((UCP_RECV_SUPP << 4) | (UCP_SEND_SUPP << 3) | (SP_SUPPORT << 2) | \
	 (RAP_SUPPORT << 1) | (RCP_SUPPORT))
