...
// Add code for check 4K downscale supported timing , If support then do
// downscale or > 3.2G timing setting downscale will be output unnormal case
...
_CODE iTE6805_SupportVT iTE6805_SupportedDSTiming[] = {
	...
	{3840, 2160, 5280, 2250, 297000, 1056, 88, 296, 8, 10, 72, 25000, PROG,
	 Vpos, Hpos, 1, "CEA Fmt94 3840x2160P@25Hz/50Hz", 94},
	/* {3840,2160,5280,2250,594000,1056,88,296,8,10,72,50000,PROG,Vpos,Hpos,1,AR_16_9,"CEA
	 */
	/* Fmt96 3840x2160P@50Hz",96}, */
	/* {3840,2160,5280,2250,297000,1056,88,296,8,10,72,25000,PROG,Vpos,Hpos,1,AR_64_27,"CEA
	 */
	/* Fmt104 3840x2160P@25Hz",104}, */
	/* {3840,2160,5280,2250,594000,1056,88,296,8,10,72,50000,PROG,Vpos,Hpos,1,AR_64_27,"CEA
	 */
	/* Fmt106 3840x2160P@50Hz",106}, */

	{4096, 2160, 5280, 2250, 297000, 968, 88, 128, 8, 10, 72, 25000, PROG, Vpos,
	 Hpos, 1, "CEA Fmt99 4096x2160P@25Hz/50Hz", 99},
	/* {4096,2160,5280,2250,594000,968,88,128,8,10,72,50000,PROG,Vpos,Hpos,1,AR_256_135,"CEA
	 */
	/* Fmt101 4096x2160P@50Hz",101}, */

	{3840, 2160, 4400, 2250, 297000, 176, 88, 296, 8, 10, 72, 30000, PROG, Vpos,
	 Hpos, 1, "CEA Fmt95 3840x2160P@30Hz/60Hz", 95},
	/* {3840,2160,4400,2250,297000,176,88,296,8,10,72,30000,PROG,Vpos,Hpos,1,AR_64_27,"CEA
	 */
	/* Fmt105 3840x2160P@30Hz",105}, */
	/* {3840,2160,4400,2250,594000,176,88,296,8,10,72,60000,PROG,Vpos,Hpos,1,AR_16_9,"CEA
	 */
	/* Fmt97 3840x2160P@60Hz",97}, */
	/* {3840,2160,4400,2250,594000,176,88,296,8,10,72,60000,PROG,Vpos,Hpos,1,AR_64_27,"CEA
	 */
	/* Fmt107 3840x2160P@60Hz",107}, */

	{4096, 2160, 5500, 2250, 297000, 1020, 88, 296, 8, 10, 72, 24000, PROG,
	 Vpos, Hpos, 1, "CEA Fmt98 4096x2160P@24Hz", 98},

	{3840, 2160, 5500, 2250, 297000, 1276, 88, 296, 8, 10, 72, 24000, PROG,
	 Vpos, Hpos, 1, "CEA Fmt93 3840x2160P@24Hz", 93},
	/* {3840,2160,5500,2250,297000,1276,88,296,8,10,72,24000,PROG,Vpos,Hpos,1,AR_64_27,"CEA
	 */
	/* Fmt103 3840x2160P@24Hz",103}, */

	{4096, 2160, 4400, 2250, 297000, 88, 88, 128, 8, 10, 72, 30000, PROG, Vpos,
	 Hpos, 1, "CEA Fmt100 4096x2160P@30Hz/60Hz", 100},
	/* {4096,2160,4400,2250,594000,88,88,128,8,10,72,60000,PROG,Vpos,Hpos,1,AR_256_135,"CEA
	 */
	/* Fmt102 4096x2160P@60Hz",102}, */
};
#define SupportVTCount \
	(sizeof(iTE6805_SupportedDSTiming) / sizeof(iTE6805_SupportVT))

// Block Set
void iTE6805_Set_AVMute(iTE_u8 AVMUTE_STATE)
{
	/* 	1C5 */
	/* 		Reg_TriSVDIO	4:0	1: Tri-state Video data IO when single pixel
	 * mode
	 */
	/* R/W	11111 		Reg_TriSVDLLIO	5	1: Tri-state Video data IO when
	 * single
	 */
	/* pixel mode	R/W	1 		Reg_TriSVDLHIO	6	1: Tri-state Video data IO
	 * when
	 */
	/* single pixel mode	R/W	1 */
	/* 		Reg_VIOSel	7	1: VIO Tri-state controlled by VIO setting */
	/* 						0: VIO Tri-state controlled by VDIO setting	R/W	1 */
	/* 	1C6	Reg_TriDVDIO	5:0	1: Tri-state Video data IO when dual pixel mode
	 */
	/* R/W	111111 */
	/* 		Reg_TriDVDLLIO	6	1: Tri-state Video data IO when dual pixel mode
	 */
	/* R/W	1 		Reg_TriVIO	7	1: Tri-state Video control IO	R/W	1 */

	/* 	4F	RegDisVAutoMute	7	1: disable video auto mute */
	/* 		RegVDGatting	5	Enable output data gating to zero when no Video
	 */
	/* display */
	...
	/* so set 3 set the output black clock (no data), and set VIOSel (sync still
	 */
	/* go on) or it will reSync(Check Stable) again. If AV Mute On, hardware */
	/* auto AV mute mute go before SW or it will be a short delay for shut the
	 */
	/* screen block so Hardware auto AVMute on -> disable AVMute for triiger */
	/* AVMute again. */
	if (AVMUTE_STATE == AVMUTE_ON) {
		chgbank(0);
		hdmirxset(0x4F, BIT5 + BIT7,
				  BIT5 + BIT7); /* enable VD Gatting , RegDisVAutoMute : 1: */
								/* Disable video auto mute */
		chgbank(1);
		hdmirxset(0xC5, BIT7,
				  BIT7); /* Reg_VIOSel  :  1: VIO Tri-state controlled by VIO */
		/* setting, 0: VIO Tri-state controlled by VDIO setting */
		chgbank(0);
		...
		if (iTE6805_DATA.STATEV == STATEV_VidStable) {
			if (iTE6805_Check_AVMute()) {
				...
				hdmirxset(
					0x4F, BIT7,
					BIT7); /* RegDisVAutoMute : 1: Disable video auto mute */
			} else {
				/* clear AV Mute */
				chgbank(1);
				...
				hdmirxset(0x4F, BIT5 | BIT7,
						  BIT5 | BIT7);				/* enable VD Gatting */
				hdmirxset(0x4F, BIT5 | BIT7, BIT7); /* disable VD Gatting */
			}
		}
		/* **************wanpeng add***************** */
		if (gpio_is_valid(it6807_config_info.backlight_gpio)) {
			if ((gpio_get_value(it6807_config_info.backlight_gpio) == 0) &&
				(iTE6805_CurVTiming.TMDSCLK == 594816)) {
...
void iTE6805_Set_ColorDepth(void)
{
	 ...
	 */
	/* Reg5D9); HDMIRX_DEBUG_PRINT("VBOTX StateA=%2X , StateB=%2X\n",(int) */
	/* (Reg5DA&0x0F),(int) (Reg5DA&0xF0)>>4); HDMIRX_DEBUG_PRINT("VBOTX */
	/* Reg5DB=%2X \n",(int) Reg5DB); */
...
void iTE6805_Set_TTL_Video_Path(void)
{
	...
	/* Reg1C1 [0] TTL half bus DDR mode , [1] TTL full bus DDR mode , [5] Enable
	 */
	/* sync embedded , [6] Enable BTA1004 mode BTA1004 mode must SyncEmb */
	...
	/* Color Depth Setting depend on input color depth , do not need setting */
	/* here. */
}
...
void iTE6805_Set_DNScale(void)
{
	...
	if (TG_HDEW == 2048) {
		...
		TG_HFP = ((iTE6805_CurVTiming.HTotal / 2) << Double) - (TG_HBP) -
				 (TG_HSW)-1920;
	...
	VIDEOTIMNG_DEBUG_PRINTF(
		"Target HActive= %d , HFrontPorch=%d, HSync Width=%d, HBackPorch=%d \n",
		TG_HDEW, TG_HFP, TG_HSW, TG_HBP);
	VIDEOTIMNG_DEBUG_PRINTF(
		"Target VActive= %d , VFrontPorch=%d, VSync Width=%d, VBackPorch=%d \n",
	...
	hdmirxwr(0x31,
			 0x10 + (0x02 << 2) + (TG_HSPOL << 1) +
				 TG_VSPOL); /* this color depth force set to 12bits Reg31 2:3 */
	...
	VIDEOTIMNG_DEBUG_PRINTF(
		"---- Video DownScale Enable! Video DownScale Enable! ----\n");
...
void iTE6805_Get_AVIInfoFrame_Info(void)
{
	...
	iTE6805_DATA.AVIInfoFrame_Input_ColorFormat =
		(AVI_Color_Format)((hdmirxrd(REG_RX_AVI_DB1) & 0x60) >>
						   5); /* CEA page 43 RGB/YCbCr */
	iTE6805_DATA.AVIInfoFrame_Colorimetry =
		(AVI_Colormetry)((hdmirxrd(REG_RX_AVI_DB2) & 0xC0) >>
						 6); /* 10=IT709 11=ExtendedColorimetry */
	iTE6805_DATA.AVIInfoFrame_ExtendedColorimetry =
		((hdmirxrd(REG_RX_AVI_DB3) & 0x70) >>
		 4); /* 000=xvYUV601, 001=xvYUV709, 010=sYUV601 .... */
	iTE6805_DATA.AVIInfoFrame_RGBQuantizationRange =
		(AVI_RANGE_Type_RGB)((hdmirxrd(REG_RX_AVI_DB3) & 0x0C) >>
							 2); /* 00 = Depend on Video Format, 01 = Limit, 10
								  */
								 /* = Full */
	iTE6805_DATA.AVIInfoFrame_VIC = (hdmirxrd(REG_RX_AVI_DB4) & 0x7F);
	iTE6805_DATA.AVIInfoFrame_YUVQuantizationRange =
		(AVI_RANGE_Type_YUV)((hdmirxrd(REG_RX_AVI_DB5) & 0xC0) >>
							 6); /* 00 = Limit Range, 01 = Full Range */
	iTE6805_DATA.AVIInfoFrame_ScanInfo = (hdmirxrd(REG_RX_AVI_DB1) & 0x03);
	chgbank(0);

	if (iTE6805_DATA.AVIInfoFrame_RGBQuantizationRange ==
		RGB_RANGE_DEPENDED_ON_VIC) {
		if (iTE6805_DATA.AVIInfoFrame_VIC >= 2) {
			iTE6805_DATA.AVIInfoFrame_RGBQuantizationRange =
				RGB_RANGE_LIMIT; /* CE Mode */
		} else {
			iTE6805_DATA.AVIInfoFrame_RGBQuantizationRange =
				RGB_RANGE_FULL; /* IT mode */
		}
	}
...
void iTE6805_Reset_ALL_Logic(iTE_u8 PORT_NUM)
{
	...
	/* 			Reg_P0_DCLKRst		3	1: Port 0 CLKD10 domain logic reset	R/W
	 */
	/* 0 */
...
// Auto Reset when B0
		...
		if (iTE6805_DATA.ChipID == 0xA0 || iTE6805_DATA.ChipID == 0xA1) {
			hdmirxset(0xC5, BIT4, BIT4); /* Unplug need to reset EDID, because
											EDID reset is also */
			/* reset SCDC in hardware, or SCDC may hang in sometimes */
			delay1ms(1);
			hdmirxset(0xC5, BIT4, 0x00); /* Unplug need to reset EDID, because
											EDID reset is also */
			/* reset SCDC in hardware, or SCDC may hang in sometimes */
		}
#endif
...
{
	chgbank(0);
	/* HDMIRX_DEBUG_PRINT("\n\niTE6805_Check_PORT0_IS_MHL_Mode = MODE_HDMI */
	/* hdmirxrd(0x13)=0x%02X !!!\n", (int) hdmirxrd(0x13)); */
	if (PORT_NUM == PORT0) {
...
iTE_u8 iTE6805_Check_AUDT(void)
{
	...
	chgbank(0);
	return (
		(!((hdmirxrd(0x10) & 0x80) >> 7)) &
		((hdmirxrd(0xB1) & 0x80) >> 7)); /* fix for cross complier warning */
}
...
void iTE6805_Set_Video_Tristate(
	iTE_u8 TRISTATE_STATE) /* TRISTATE_ON, TRISTATE_OFF */
{
	chgbank(0);
#if (COLOR_DEPTH_BY_GCPINFOFRAME == TRUE)
	iTE6805_CurVTiming.ColorDepth = (hdmirxrd(0x98) & 0xF0);
	...
	if (TRISTATE_STATE == TRISTATE_OFF) {
		...
		if (iTE6805_Check_Single_Dual_Mode() == MODE_SINGLE) {
			...
			case 0x40:
				...
				switch (iTE6805_CurVTiming.ColorDepth) {
				case 0x10:				  /* 8b */
					hdmirxwr(0xC5, 0x78); /* turn off tristate If IO mode do */
					/* not need it, only for single mode */
					break;
				case 0x20:				  /* 10b */
					hdmirxwr(0xC5, 0x38); /* turn off tristate If IO mode do */
					/* not need it, only for single mode */
					break;
				default:				  /* 12b+ */
					hdmirxwr(0xC5, 0x18); /* turn off tristate If IO mode do */
					/* not need it, only for single mode */
					break;
				}
#else
#if (Output_Color_Depth == 8)
				hdmirxwr(0xC5, 0x78); /* turn off tristate If IO mode do not */
									  /* need it, only for single mode */
#elif (Output_Color_Depth == 10)
				hdmirxwr(0xC5, 0x38); /* turn off tristate If IO mode do not */
									  /* need it, only for single mode */
#elif (Output_Color_Depth == 12)
				hdmirxwr(0xC5, 0x18); /* turn off tristate If IO mode do not */
									  /* need it, only for single mode */
#endif
#endif
			...
			case 0x80:
				...
				switch (iTE6805_CurVTiming.ColorDepth) {
				case 0x10:				  /* 8b */
					hdmirxwr(0xC5, 0x63); /* turn off tristate If IO mode do */
					/* not need it, only for single mode */
					break;
				case 0x20:				  /* 10b */
					hdmirxwr(0xC5, 0x23); /* turn off tristate If IO mode do */
					/* not need it, only for single mode */
					break;
				default:				  /* 12b+ */
					hdmirxwr(0xC5, 0x03); /* turn off tristate If IO mode do */
					/* not need it, only for single mode */
					break;
				}
#else
// setting Tristate IO by IO/Mode & Color Depth
#if (Output_Color_Depth == 8)
				hdmirxwr(0xC5, 0x63); /* turn off tristate If IO mode do not */
									  /* need it, only for single mode */
#elif (Output_Color_Depth == 10)
				hdmirxwr(0xC5, 0x23); /* turn off tristate If IO mode do not */
									  /* need it, only for single mode */
#elif (Output_Color_Depth == 12)
				hdmirxwr(0xC5, 0x03); /* turn off tristate If IO mode do not */
									  /* need it, only for single mode */
#endif
#endif
			...
			switch (iTE6805_CurVTiming.ColorDepth) {
			case 0x10:				  /* 8b */
				hdmirxwr(0xC6, 0x40); /* turn off tristate If IO mode do not */
									  /* need it, only for single mode */
				break;
			default:				  /* 10b+ */
				hdmirxwr(0xC6, 0x00); /* turn off tristate If IO mode do not */
									  /* need it, only for single mode */
				break;
			}
...
#else
#if (Output_Color_Depth == 8)
			hdmirxwr(0xC6, 0x40); /* turn off tristate If IO mode do not need */
								  /* it, only for single mode */
#elif (Output_Color_Depth == 10 || Output_Color_Depth == 12)
			hdmirxwr(0xC6, 0x00); /* turn off tristate If IO mode do not need */
								  /* it, only for single mode */
#endif
#endif
		}
		HDMIRX_VIDEO_PRINTF(
			"---------------- iTE6805_Set_Video_Tristate -> VIDEO_ON = "
			"Tristate off\n");
	} else {
		chgbank(1);
		hdmirxwr(0xC5, 0xFF);
		hdmirxwr(0xC6, 0xFF);
		HDMIRX_VIDEO_PRINTF(
			"---------------- iTE6805_Set_Video_Tristate -> VIDEO_OFF = "
			"Tristate on\n");
	}
	chgbank(0);
}
...
iTE_u16 iTE6805_Get_VIC_Number()
{
	...
	for (i = 0; i<(SizeofVMTable - 1); i++) {
		if ((abs(iTE6805_CurVTiming.PCLK - s_VMTable[i].PCLK)>(s_VMTable[i].PCLK
* 5 / 100)) || (iTE6805_CurVTiming.HActive != s_VMTable[i].HActive) ||
...
void iTE6805_Get_PCLK(void)
{
	...
	PCLK = RCLKVALUE * 512 * 5 /
		   sump; /* 512=2*256 because of 1T 2 pixel, *5 = for repeat times */
...
void iTE6805_Get_TMDS(iTE_u8 count)
{
	...
	/* (RCLKVALUE % 1000) / 10); HDMIRX_DEBUG_PRINT("rddata=%d \n", (int) */
	/* rddata); */
...
void iTE6805_Show_AVIInfoFrame_Info(void)
{
	...
	VIDEOTIMNG_DEBUG_PRINTF("AVI Info Frame Input ColorFormat = %X ",
	...
	VIDEOTIMNG_DEBUG_PRINTF("AVI Info Frame Colorimetry = %X",
							(int)iTE6805_DATA.AVIInfoFrame_Colorimetry);
	...
	VIDEOTIMNG_DEBUG_PRINTF(
		"AVI Info Frame RGBQuantizationRange = %X",
		(int)iTE6805_DATA.AVIInfoFrame_RGBQuantizationRange);
	...
	VIDEOTIMNG_DEBUG_PRINTF(
		"AVI Info Frame YUVQuantizationRange = %X",
		(int)iTE6805_DATA.AVIInfoFrame_YUVQuantizationRange);
...
void chgbank(iTE_u16 bank) { hdmirxset(0x0F, 0x07, bank & 7); }
...
void iTE6805_OCLK_Cal(void)
{
...
#if (iTE6807 == FALSE)
	HDMIRX_DEBUG_PRINT(
		"After Adjust, OSCCLK=%ld.%03ldMHz, diff to 39.000MHz .....\n",
	...
	CEC_timeunit =
		(iTE_u8)(RCLKVALUE / 160); /* use RCLK/160 instead  of OCLK/320 , */
								   /* because timeunit last step came to RCLK */
	HDMIRX_DEBUG_PRINT("CEC_timeunit (T100us) = %X\n", (int)CEC_timeunit);
#endif
	temp = RCLKVALUE;
	RCLKVALUE = RCLKVALUE + RCLKVALUE / 10; /* 20170327 MHL CTS */
	HDMIRX_DEBUG_PRINT("RCLKVALUE=%ld.%02ldMHz\n", (long int)(RCLKVALUE / 1000),
					   (long int)((RCLKVALUE % 1000) / 10));
	...
	/* 2017/11/24 add for B0/B1 chip by Emily mail(B1 Software change: SCDC RR
	 * relative register) */
	...
	if (iTE6805_DATA.ChipID == 0xB1) {
		...
		hdmirxwr(0xFD, Xp);
		hdmirxset(0xFE, 0x20,
				  0x00); /* bus free wait time when rr test dly<4 ms */
		hdmirxset(0xFE, 0x0F, 0x0C); /* 1msto timer (D[3:0]* 80us) <0xC0 */
		hdmirxset(0xFE, 0x10, 0x10); /* use internal 10ms timer */
...
iTE_u32 iTE6805_OCLK_Load(void)
{
	...
	if (temp[0] == 0xFF && temp[1] == 0xFF && temp[2] == 0x00 &&
		temp[3] == 0x00) {
		Read_Blcok = 0x04;
	} else {
		Read_Blcok = 0x00;
	...
	if ((lTemp > iTE6807_OSC_MAX_VALUE) || (lTemp < iTE6807_OSC_MIN_VALUE)) {
		HDMIRX_DEBUG_PRINT(
			" Readback value invalid: 0x%08lX, use typical value instead.\n",
...
void iTE6805_Init_fsm(void)
{
	...
	/* init EDID */
	iTE6805_DATA.CurrentPort = 0; /* !!!!!! SOME OS/MCU NOT INIT GLOBAL
									 VARIABLE, NEED TO INIT BY CODE */
	/* OR MAY INIT ERROR WHEN GOT EDID ARRAY POINTER !!!!!! */
	iTE6805_EDID_Init();
...
#else
	iTE6805_HDCP_EnableRepeaterMode(
		_ENABLE_6805_AS_HDCP_REPEATER_); /* will keep stay in repeater mkdir */
#endif
#endif
...
void iTE6805_Init_CAOF(void)
{
	...
	hdmirxwr(0x25, 0x00);		 /* 	able AFE PWD */
	hdmirxwr(0x26, 0x00);
	hdmirxwr(0x27, 0x00);
	hdmirxwr(0x28, 0x00);
...
void iTE6807_Set_EnSSC(void)
{
	...
	if (result < 46) {
		...
		hdmirxset(0xE3, 0x08,
				  0x00); /* pccmd s e3 08 00 90; :: [3] Reg_Spread : disable */
		hdmirxset(0xD0, 0xF0,
				  0xD0); /* pccmd s d0 f0 d0 90; :: [7] SP_DEI, [6] */
						 /* EnSPBROvWr, [5:4]: SP_BR[1:0] */
		hdmirxset(0xD0, 0x0F, 0x0C); /* pccmd s d0 0f 0c 90; :: [3:0] */
		/* SP_PWDB/SP_RESETB/SoftSPSDMRst/GateSPSDMCLK */
		hdmirxset(0xD1, 0x08, 0x00); /* pccmd s d1 08 00 90; :: [3] : SP_BYS */
		hdmirxset(0xE3, 0x07,
				  0x00);	  /* pccmd s e3 07 00 90; :: [2:0] SDMInc[10:8] */
		...
		hdmirxset(0xE3, 0x08,
				  0x08); /* pccmd s e3 08 08 90; :: [3] Reg_Spread : enable */
		...
		hdmirxset(0xE3, 0x08,
				  0x00); /* pccmd s e3 08 00 90; :: [3] Reg_Spread : disable */
		hdmirxset(0xD0, 0xF0,
				  0xE0); /* pccmd s d0 f0 e0 90; :: [7] SP_DEI, [6] */
						 /* EnSPBROvWr, [5:4]: SP_BR[1:0] */
		hdmirxset(0xD0, 0x0F, 0x0C); /* pccmd s d0 0f 0c 90; :: [3:0] */
		/* SP_PWDB/SP_RESETB/SoftSPSDMRst/GateSPSDMCLK */
		hdmirxset(0xD1, 0x08, 0x00); /* pccmd s d1 08 00 90; :: [3] : SP_BYS */
		...
		hdmirxset(0xE3, 0x08,
				  0x00); /* pccmd s e3 08 00 90; :: [3] Reg_Spread : disable */
		hdmirxset(0xD0, 0xF0,
				  0xF0); /* pccmd s d0 f0 f0 90; :: [7] SP_DEI, [6] */
						 /* EnSPBROvWr, [5:4]: SP_BR[1:0] */
		hdmirxset(0xD0, 0x0F, 0x0C); /* pccmd s d0 0f 0c 90; :: [3:0] */
		/* SP_PWDB/SP_RESETB/SoftSPSDMRst/GateSPSDMCLK */
		hdmirxset(0xD1, 0x08, 0x00); /* pccmd s d1 08 00 90; :: [3] : SP_BYS */
...
void iTE6805_Set_Power_Mode(iTE_u8 Mode)
{
	...
	if (Mode == MODE_POWER_SAVING) {
		...
		hdmirxset(0xD0, 0x0F,
				  0x03); /* SP_PWDB&SP_RESETB&SPSDMRst&GateSPSDMCLK */
#else
		...
		hdmirxset(0xA8, 0x08, 0x00);
		hdmirxwr(0x0F, 0x00);
		hdmirxwr(0x21, 0xFB); /* hdmirxwr(0x21, 0xFB);	for Enable CEC  when
								 powersaving , */
		/* hdmirxwr(0x21, 0xFF);	for Disable CEC when powersaving */
		hdmirxwr(0x20, 0xE9); /* 20181219 fix for EDID need to change even if */
							  /* power saving, can't power down it */
#if (DYNAMIC_HDCP_ENABLE_DISABLE == TRUE)
	...
	if (Mode == MODE_POWER_NORMAL) {
		...
		hdmirxset(0xD0, 0x0F,
				  0x0C); /* SP_PWDB&SP_RESETB&SPSDMRst&GateSPSDMCLK */
#else
