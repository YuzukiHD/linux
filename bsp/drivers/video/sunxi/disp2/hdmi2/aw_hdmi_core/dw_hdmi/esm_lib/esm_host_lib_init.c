...
ESM_STATUS ESM_LoadPairing(esm_instance_t *esm, uint8_t *PairData, uint32_t BufferSize)
{
	...
	if (PairData && BufferSize) {
		...
		if (err != ESM_HL_DRIVER_SUCCESS) {
			hdmi_err("%s: Data WRITE Failed %d\n", __func__, err);
...
ESM_STATUS ESM_SavePairing(esm_instance_t *esm, uint8_t *PairData, uint32_t *BufferSize)
{
	...
	if (*BufferSize < esm->esm_pair_size) {
		hdmi_err("%s: BufferSize:%d esm_pair_size:%d\n",
			__func__, *BufferSize, esm->esm_pair_size);
				...
				uint32_t *offset)
{
	...
	if (ESM_APP_CONFIG_MAX_SIZE > ESM_CONFIG_MAX_SIZE) {
		hdmi_err("%s: ESM_APP_CONFIG_MAX_SIZE(%lx) > ESM_CONFIG_MAX_SIZE(%x)\n",
				__func__, ESM_APP_CONFIG_MAX_SIZE, ESM_CONFIG_MAX_SIZE);
		return ESM_FAILED;
	}
	...
	if (sys_c->cookie != ESM_CONFIG_COOKIE) {
		hdmi_err("%s: ESM_CONFIG_BAD_COOKIE\n", __func__);
	...
	if (fw_version != ESM_HDCP_HDMI_UNKNOWN_ID) {
		if (sys_c->fw_version != fw_version) {
			hdmi_err("%s: ESM_CONFIG_FW_SYS_VERSION_NOT_SUPPORTED\n", __func__);
	...
	if (sys_c->watchdog_mode >= ESM_SYSTEM_WATCHDOG_MODE_MAX) {
		hdmi_err("%s: watchdog_mode ESM_CONFIG_WD_ENUM_NOT_SUPPORTED\n", __func__);
	...
	if (sys_c->logging_mode >= ESM_SYSTEM_LOGGING_MODE_MAX) {
		hdmi_err("%s: logging_mode ESM_CONFIG_WD_ENUM_NOT_SUPPORTED\n", __func__);
		return ESM_CONFIG_WD_ENUM_NOT_SUPPORTED;
	}
	...
	if (sys_c->logging_level  >= ESM_SYSTEM_LOGGING_LEVEL_MAX) {
		hdmi_err("%s: ESM_CONFIG_LOGGING_LEVEL_NOT_SUPPORTED\n", __func__);
	...
	if (sys_c->c_int.ext_mem_total_size >= ESM_SYSTEM_MAX_SIZE) {
		hdmi_err("%s: ESM_CONFIG_MEM_SIZE_NOT_SUPPORTED\n", __func__);
	...
	if (sys_c->c_int.ext_mem_mb_size >= ESM_SYSTEM_MB_MAX_SIZE) {
		hdmi_err("%s: ESM_CONFIG_MB_SIZE_NOT_SUPPORTED\n", __func__);
	...
	if ((sys_c->c_int.ext_mem_except_size >= ESM_SYSTEM_EXCEPT_MAX_SIZE) ||
		...
		hdmi_err("%s: ESM_CONFIG_EXCEPT_SIZE_NOT_SUPPORTED\n", __func__);
	...
	if (sys_c->c_int.ext_mem_srm_size >= ESM_SYSTEM_SRM_MAX_SIZE) {
		hdmi_err("%s: ESM_CONFIG_SRM_SIZE_NOT_SUPPORTED\n", __func__);
	...
	if (sys_c->c_int.ext_mem_topo_size >= ESM_SYSTEM_TOPO_MAX_SIZE) {
		hdmi_err("%s: ESM_CONFIG_TOPO_SIZE_NOT_SUPPORTED\n", __func__);
	...
	if (sys_c->c_int.ext_mem_pairing_size >= ESM_SYSTEM_PAIRING_MAX_SIZE) {
		hdmi_err("%s: ESM_CONFIG_PAIRING_SIZE_NOT_SUPPORTED\n", __func__);
	...
	if (sys_c->c_int.ext_mem_logging_size >= ESM_SYSTEM_LOGGING_MAX_SIZE) {
		hdmi_err("%s: ESM_CONFIG_LOGGING_SIZE_NOT_SUPPORTED\n", __func__);
	...
	if (sys_c->c_int.ext_mem_topo_seed_size > ESM_HDCP_HDMI_TOPO_INTEGRITY_KEY_SIZE) {
		hdmi_err("%s: ESM_CONFIG_TOPO_SEED_SIZE_NOT_SUPPORTED\n", __func__);
...
/* Ensure total memory size is greater than all the sub-partition sizes. */
	if (sys_c->c_int.ext_mem_total_size < (uint32_t)(sys_c->c_int.ext_mem_cfgdata_size +
		...
		hdmi_err("%s: ESM_CONFIG_VERIFY_FAILED\n", __func__);
							...
							uint32_t image_size)
{
	...
	if (esm->driver->hpi_write(esm->driver->instance,
		...
		hdmi_err("%s: Failed to write code address 0x%x\n",
				__func__, ESM_REG_HP_FW_BASE0(HPI_HOST_OFF));
	...
	if (esm->driver->hpi_read(esm->driver->instance,
		...
		hdmi_err("%s: Failed to read code address 0x%x\n",
				__func__, ESM_REG_HP_FW_BASE0(HPI_HOST_OFF));
		return ESM_HL_FAILED_TO_SET_CODE_ADDR;
	}
	...
	if (addr != esm->driver->code_base) {
		hdmi_err("%s: Code address mismatch, code_phy_addr:%x ESM code addr:%x\n",
				__func__, esm->driver->code_base, addr);
		return ESM_HL_FAILED_TO_SET_CODE_ADDR;
	}
	...
	if (esm->driver->hpi_write(esm->driver->instance,
		...
		hdmi_err("%s: Failed to write data address 0x%x\n",
				__func__, ESM_REG_HP_DATA_BASE0(HPI_HOST_OFF));
	...
	if (esm->driver->hpi_read(esm->driver->instance,
		...
		hdmi_err("%s: Failed to read data address 0x%x\n",
				__func__, ESM_REG_HP_DATA_BASE0(HPI_HOST_OFF));
		return ESM_HL_FAILED_TO_SET_DATA_ADDR;
	}
	...
	if (addr != esm->driver->data_base) {
		hdmi_err("%s: Data address mismatch, data_phy_addr:%x ESM code addr:%x\n",
				__func__, esm->driver->data_base, addr);
		return ESM_HL_FAILED_TO_SET_DATA_ADDR;
	}
	/* Set FW_VLD bit. */
	if (esm->driver->hpi_write(esm->driver->instance, HP_CTRL, 0x1) != ESM_HL_DRIVER_SUCCESS) {
		hdmi_err("%s: Failed to set FW_VLD bit\n", __func__);
	...
	if (esm_hostlib_wait_mb_response(esm, CMD_DEFAULT_TIMEOUT) != ESM_HL_SUCCESS) {
		hdmi_err("%s: Failed to set FW_VLD bit (no response)\n", __func__);
		return ESM_HL_FAILED_TO_SET_VLD_BIT;
	}
	...
	if (esm->driver->hpi_read(esm->driver->instance,
		...
		hdmi_err("%s: Failed to read FW_VLD [%d]\n", __func__, err);
	...
	if (esm->driver->hpi_read(esm->driver->instance,
		...
		hdmi_err("%s: Failed to read FW_VERSION [%d]\n", __func__, err);
		return ESM_HL_DRIVER_HPI_READ_FAILED;
	}
	...
	if (esm->driver->hpi_read(esm->driver->instance,
		...
		hdmi_err("%s: Failed to read DESIGN_ID [%d]\n", __func__, err);
		return ESM_HL_DRIVER_HPI_READ_FAILED;
	}
	...
	if (esm->driver->hpi_read(esm->driver->instance,
		...
		hdmi_err("%s: Failed to read PRODUCT_ID [%d]\n", __func__, err);
		return ESM_HL_DRIVER_HPI_READ_FAILED;
	}
	...
	if (esm->driver->hpi_read(esm->driver->instance,
		...
		hdmi_err("%s: Failed to read FW_INFO [%d]\n", __func__, err);
		return ESM_HL_DRIVER_HPI_READ_FAILED;
	}
...
ESM_STATUS esm_get_configuration(esm_instance_t *esm, esm_config_t *returned_config)
{
	...
	if (esm->driver->data_read(esm->driver->instance, 0,
		...
		hdmi_err("%s: Failed to read configuration data from memory [%d]\n", __func__, err);
	...
	if (esm_image_read(config, 0, 0, 0, &esm->esm_ctrl,
		...
		hdmi_err("%s: Failed to parse configuration data [%d]\n", __func__, err);
	...
	case ESM_HDCP_HDMI_TX_ID:
		...
		HDCP_INF("\tconfig_version\t\t= %d (TX)\n", esm->app_id);

		if (esm_image_read_tx(config + offset, &esm->hdcp_c, &offset) != ESM_SUCCESS) {
			hdmi_err("%s: [ESM_HDCP_HDMI_TX_ID] Failed to parse configuration data [%d]\n",
					__func__, err);
			return ESM_HL_FAILED_TO_PARSE_CONFIG_DATA;
		}

		HDCP_INF("\tpairing_enabled\t\t= %d (TX)\n", esm->hdcp_c.c_int.pairing_enabled);
		HDCP_INF("\tmax_pairing_devices\t= %d (TX)\n", esm->hdcp_c.c_int.max_pairing_devices);
		HDCP_INF("\tlvc_timeout\t\t= %d ms (TX)\n", esm->hdcp_c.c_int.tx_lvc_timeout);
		HDCP_INF("\tlvc_ignore_timeout\t= %d ms (TX)\n",
			esm->hdcp_c.c_int.tx_lvc_ignore_timeout);
		break;
	default:
		hdmi_wrn("%s: invalid app_id %d\n", __func__, esm->app_id);
		break;
	}
	...
	HDCP_INF("\t------------------------------\n");
	HDCP_INF("\texcept_mode\t\t= %x\n", esm->esm_ctrl.c_int.except_mode);
	HDCP_INF("\text_mem_total_size\t= %d\n", esm->esm_ctrl.c_int.ext_mem_total_size);
	HDCP_INF("\text_mem_cfgdata_size\t= %d\n", esm->esm_ctrl.c_int.ext_mem_cfgdata_size);
	HDCP_INF("\text_mem_cpireg_size\t= %d\n", esm->esm_ctrl.c_int.ext_mem_cpireg_size);
	HDCP_INF("\text_mem_mb_size\t\t= %d\n", esm->esm_ctrl.c_int.ext_mem_mb_size);
	HDCP_INF("\text_mem_except_size\t= %d\n", esm->esm_ctrl.c_int.ext_mem_except_size);
	HDCP_INF("\text_mem_logging_size\t= %d\n", esm->esm_ctrl.c_int.ext_mem_logging_size);
	HDCP_INF("\text_mem_srm_size\t= %d\n", esm->esm_ctrl.c_int.ext_mem_srm_size);
	HDCP_INF("\text_mem_topo_size\t= %d\n", esm->esm_ctrl.c_int.ext_mem_topo_size);
	HDCP_INF("\text_mem_topo_seed_size\t= %d\n", esm->esm_ctrl.c_int.ext_mem_topo_seed_size);
	HDCP_INF("\text_mem_pairing_size\t= %d\n", esm->esm_ctrl.c_int.ext_mem_pairing_size);
	HDCP_INF("\twoocompatability\t= %d\n", esm->hdcp_c.woocompatability);
	HDCP_INF("\take_timeouts\t\t= %d\n", esm->hdcp_c.c_int.ake_timeouts);
	HDCP_INF("\ti2c_freq bitrate\t= %d khz\n", esm->esm_ctrl.i2c_freq);
	HDCP_INF("\tcee_enable\t\t= %d\n", esm->hdcp_c.c_int.cee_enable);
	HDCP_INF("\tcee_state_wait_time\t= %d us\n", esm->esm_ctrl.cee_state_wait_time);

	HDCP_INF("\tesm_cpi_off\t\t= %d\n", esm->esm_cpi_off);
	HDCP_INF("\tesm_mb_off\t\t= %d\n", esm->esm_mb_off);
	HDCP_INF("\tesm_exc_off\t\t= %d\n", esm->esm_exc_off);
	HDCP_INF("\tesm_log_(header)_off\t= %d\n", esm->esm_log_header_off);
	HDCP_INF("\tesm_srm_off\t\t= %d\n", esm->esm_srm_off);
	HDCP_INF("\tesm_topo_off\t\t= %d\n", esm->esm_topo_off);
	HDCP_INF("\tesm_topo_slots\t\t= %d\n", esm->esm_topo_slots);
	HDCP_INF("\tesm_topo_seed_off\t= %d\n", esm->esm_topo_seed_off);
	HDCP_INF("\tesm_pair_off\t\t= %d\n", esm->esm_pair_off);
	HDCP_INF("\tesm_pair_size\t\t= %d\n", esm->esm_pair_size);
	...
	esm_host_driver_t *driver, esm_config_t *config)
{
	...
	if (esm_init_mem(esm, image, image_size) != ESM_HL_SUCCESS) {
		hdmi_err("%s: esm init mem failed!\n", __func__);
		return -1;
	}
	/* return ESM_HL_SUCCESS; */
