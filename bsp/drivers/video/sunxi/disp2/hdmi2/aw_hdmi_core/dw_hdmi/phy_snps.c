...
static struct phy_config301 phy301[] = {
...
static struct phy_config301 *_snps_phy_301_get_configs(u32 pClk, dw_color_depth_t color,
						...
						dw_pixel_repetition_t pixel)
{
	...
	config = _snps_phy_301_get_configs(pClk, color, pixel);
	if (config == NULL) {
	...
	snps_phy_write(dev, OPMODE_PLLCFG, config->oppllcfg);
	if (snps_phy_read(dev, OPMODE_PLLCFG, &phyRead) || (phyRead != config->oppllcfg))
		hdmi_err("%s: OPMODE_PLLCFG Mismatch Write 0x%04x Read 0x%04x\n",
				__func__, config->oppllcfg, phyRead);

	snps_phy_write(dev, PLLCURRCTRL, config->pllcurrctrl);
	if (snps_phy_read(dev, PLLCURRCTRL, &phyRead) || (phyRead != config->pllcurrctrl))
	...
	snps_phy_write(dev, PLLGMPCTRL, 3);
	snps_phy_read(dev, PLLGMPCTRL, &phyRead);
	if (phyRead != 3)
		hdmi_err("%s: PLLGMPCTRL Mismatch Write 0x%04x Read 0x%04x\n",
				__func__, config->pllgmpctrl, phyRead);
#else
	snps_phy_write(dev, PLLGMPCTRL, config->pllgmpctrl);
	snps_phy_read(dev, PLLGMPCTRL, &phyRead);
	...
	snps_phy_write(dev, TXTERM, config->txterm);
	if (snps_phy_read(dev, TXTERM, &phyRead) || (phyRead != config->txterm))
		hdmi_err("%s: TXTERM Mismatch Write 0x%04x Read 0x%04x\n",
				__func__, config->txterm, phyRead);

	snps_phy_write(dev, CKSYMTXCTRL, config->cksymtxctrl);
	if (snps_phy_read(dev, CKSYMTXCTRL, &phyRead) || (phyRead != config->cksymtxctrl))
		hdmi_err("%s: CKSYMTXCTRL Mismatch Write 0x%04x Read 0x%04x\n",
				__func__, config->cksymtxctrl, phyRead);

	snps_phy_write(dev, VLEVCTRL, config->vlevctrl);
	if (snps_phy_read(dev, VLEVCTRL, &phyRead) || (phyRead != config->vlevctrl))
	...
	if (dev->snps_hdmi_ctrl.pixel_clock == 594000) {
		snps_phy_write(dev, 0x17, 0x8006);
		if (snps_phy_read(dev, 0x17, &phyRead) || (phyRead != 0x8006))
						...
						dw_pixel_repetition_t pixel)
{
	...
	snps_phy_write(dev, OPMODE_PLLCFG, config->oppllcfg);
	snps_phy_read(dev, OPMODE_PLLCFG, &phyRead);
	if (phyRead != config->oppllcfg)
		hdmi_err("%s: OPMODE_PLLCFG Mismatch Write 0x%04x Read 0x%04x\n",
				__func__, config->oppllcfg, phyRead);

	snps_phy_write(dev, PLLCURRCTRL, config->pllcurrctrl);
	snps_phy_read(dev, PLLCURRCTRL, &phyRead);
	...
	snps_phy_write(dev, TXTERM, config->txterm);
	snps_phy_read(dev, TXTERM, &phyRead);
	if (phyRead != config->txterm)
		hdmi_err("%s: TXTERM Mismatch Write 0x%04x Read 0x%04x\n",
				__func__, config->txterm, phyRead);

	snps_phy_write(dev, CKSYMTXCTRL, config->cksymtxctrl);
	snps_phy_read(dev, CKSYMTXCTRL, &phyRead);
	if (phyRead != config->cksymtxctrl)
		hdmi_err("%s: CKSYMTXCTRL Mismatch Write 0x%04x Read 0x%04x\n",
				__func__, config->cksymtxctrl, phyRead);

	snps_phy_write(dev, VLEVCTRL, config->vlevctrl);
	snps_phy_read(dev, VLEVCTRL, &phyRead);
