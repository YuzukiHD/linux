...
s32 lvds_open(u32 sel, struct disp_panel_para *panel)
{
	...
	if (panel->lcd_lvds_if == LCD_LVDS_IF_DUAL_LINK) {
		...
		if (panel->lcd_lvds_clk_polarity)
			lcd_dev[sel]->tcon0_lvds_ana[sel].bits.pd = 3;
		else
		...
		if (panel->lcd_lvds_io_polarity) {
			lcd_dev[sel]->tcon0_lvds_ana[sel].bits.res0 = 3;
			lcd_dev[sel]->tcon0_lvds_ana[sel].bits.pwslv = 1;
			lcd_dev[sel]->tcon0_lvds_ana[sel].bits.pwsmb = 1;
		}
	}
