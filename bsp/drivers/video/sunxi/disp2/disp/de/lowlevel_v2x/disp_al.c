...
int disp_init_al(struct disp_bsp_init_para *para)
{
	...
	for (j = 0; j < DEVICE_NUM; j++) {
		if (para->boot_info[j].sync == 1) {
			u32 disp = para->boot_info[j].disp;
			u32 tcon_id;
			...
			for (hwdev_index = 0; hwdev_index < DEVICE_NUM; ++hwdev_index) {
				if (!bsp_disp_feat_is_supported_output_types(
							hwdev_index, para->boot_info[j].type)) {
					continue;
				}
				if (disp != i) {
					++i;
			 ...
			 */
			al_priv.output_type[tcon_id] = para->boot_info[j].type;
			al_priv.output_mode[tcon_id] = para->boot_info[j].mode;
