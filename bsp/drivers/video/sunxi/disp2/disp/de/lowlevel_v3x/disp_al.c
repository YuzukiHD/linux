...
int disp_init_al(struct disp_bsp_init_para *para)
{
	...
	for (i = 0; i < DEVICE_NUM; i++) {
		if (para->boot_info[i].sync == 1) {
			 ...
			 */
			al_priv.output_type[tcon_id] = para->boot_info[i].type;
			al_priv.output_mode[tcon_id] = para->boot_info[i].mode;
			al_priv.tcon_type[tcon_id] = 0;
#if defined(SUPPORT_HDMI)
			if (para->boot_info[i].type == DISP_OUTPUT_TYPE_HDMI) {
				tcon_id = tcon_id - 1;
			...
			al_priv.tcon_type[tcon_id] =
				(para->boot_info[i].type == DISP_OUTPUT_TYPE_TV) ?
