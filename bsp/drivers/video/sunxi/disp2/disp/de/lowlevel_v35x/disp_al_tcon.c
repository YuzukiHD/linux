...
int disp_al_hdmi_volume_force(u32 screen_id, u32 value)
{
	tcon1_volume_force(screen_id, value);
	return 0;
}
...
int disp_al_edp_irq_enable(u32 screen_id)
{
	tcon_irq_enable(screen_id, LCD_IRQ_TCON1_VBLK);
	return 0;
}

int disp_al_edp_irq_disable(u32 screen_id)
{
	tcon_irq_disable(screen_id, LCD_IRQ_TCON1_VBLK);
	return 0;
}
...
#if defined(SUPPORT_EDP)
	if (output_type == DISP_OUTPUT_TYPE_EDP) {
		al_priv.tcon_type[screen_id] = 1;
	}
#endif
