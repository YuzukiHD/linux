...
{
	struct de_vsu_private *priv = NULL;
	struct asu_reg *reg = NULL;
	asu_module_param_t *para = NULL;
	int i = 0;
	/* int j = 0; */

	DE_INF("sel=%d, cmd=%d, subcmd=%d, data=%px\n", sel, cmd, subcmd, data);
	para = (asu_module_param_t *)data;
	if (para == NULL) {
		DE_WRN("para NULL\n");
		return -1;
	}
	...
	for (i = 0; i < VI_CHN_NUM; i++) {
		if (de_feat_get_scaler_type(sel, i) != DE_SCALER_TYPE_ASU)
			continue;

		priv = &(vsu_priv[sel][i]);
		reg = get_asu_reg(priv);
		if (subcmd == 16) { /* read */
		...
		/* for (j = 0; j < ASU_REG_COUNT; j++) { */
			/* DE_WRN("Sunxi_PQ_Dev: para[%d] = %d\n", j, para->value[j]); */
		/* } */
	}
	return 0;
}
