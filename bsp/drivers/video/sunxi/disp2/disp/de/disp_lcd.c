...
static s32 lcd_parse_panel_para(u32 disp, struct disp_panel_para *info)
{
	...
	ret =
	    disp_sys_script_get_item(primary_key, "lcd_pwm_name",
				     (int *)info->lcd_pwm_name, 2);
...
static s32 lcd_clk_config(struct disp_device *lcd)
{
	...
	if ((pll_rate_set != pll_rate) || (lcd_rate_set != lcd_rate)
		...
		DE_WRN("lcd %d, clk: pll(%ld),clk(%ld),dclk(%ld) dsi_rate(%ld)\n ",
			lcd->disp, pll_rate, lcd_rate, dclk_rate, dsi_rate);
		DE_WRN("clk real:pll(%ld),clk(%ld),dclk(%ld) dsi_rate(%ld)\n",
...
static s32 lcd_clk_enable(struct disp_device *lcd, bool sw_enable)
{
	...
	if (ret) {
		DE_WRN("reset_control_deassert for rst_bus_tcon_lcd failed\n");
		return ret;
	}

	ret = clk_prepare_enable(lcdp->clk_tcon_lcd);
	if (ret) {
		DE_WRN("clk_prepare_enable for clk_tcon_lcd failed\n");
		return ret;
	}

	ret = clk_prepare_enable(lcdp->clk_bus_tcon_lcd);
	if (ret) {
		DE_WRN("clk_prepare_enable for clk_bus_tcon_lcd failed\n");
		return ret;
	}
	...
	if (lcdp->panel_info.lcd_if == LCD_IF_LVDS) {
		...
		if (ret) {
			DE_WRN("reset_control_deassert for rst_lvds failed\n");
			return ret;
		}
		ret = clk_prepare_enable(lcdp->clk_lvds);
		if (ret) {
			DE_WRN("clk_prepare_enable for clk_lvds failed\n");
			return ret;
		}
		...
		if (ret) {
			DE_WRN("clk_prepare_enable for clk_edp failed\n");
			return ret;
		}
	}
	...
	if (enable_mipi) {
		...
		for (i = 0; i < dsi_num; i++) {
			...
			if (ret) {
				DE_WRN("reset_control_deassert for rst_bus_mipi_dsi[%d] failed\n", i);
				return ret;
			}

			ret = clk_prepare_enable(lcdp->clk_mipi_dsi[i]);
			if (ret) {
				DE_WRN("clk_prepare_enable for clk_mipi_dsi[%d] failed\n", i);
				return ret;
			}

			ret = clk_prepare_enable(lcdp->clk_bus_mipi_dsi[i]);
			if (ret) {
				DE_WRN("clk_prepare_enable for clk_bus_mipi_dsi[%d] failed\n", i);
				return ret;
			}
	/* This clock cannot be used at FPGA stage */
			...
			if (ret) {
				DE_WRN("clk_prepare_enable for clk_mipi_dsi_combphy[%d] failed\n", i);
				return ret;
			}
#endif
		}
...
static s32 lcd_clk_disable(struct disp_device *lcd)
{
	...
	if (lcdp->panel_info.lcd_if == LCD_IF_LVDS) {
		...
		if (ret) {
			DE_WRN("reset_control_assert for rst_bus_lvds failed\n");
			return ret;
		}
#if defined(SUPPORT_DSI)
		disable_mipi = 1;
	...
	if (disable_mipi) {
		...
		for (i = 0; i < dsi_num; i++) {
			...
			if (ret) {
				DE_WRN("reset_control_assert for rst_bus_mipi_dsi failed\n");
				return ret;
			}
		}
	...
	if (ret) {
		DE_WRN("reset_control_assert for rst failed\n");
		return ret;
	}
...
static s32 disp_lcd_pwm_enable(struct disp_device *lcd)
{
	...
	if (lcdp->panel_info.lcd_pwm_used && !lcdp->pwm_info.dev) {
		if (lcdp->panel_info.lcd_pwm_name[0] == '\0')
			lcdp->pwm_info.dev =
...
static s32 disp_lcd_update_gamma_tbl_set(struct disp_device *lcd)
{
	...
	if (color_temperature < -50 || color_temperature > 150) {
		DE_WRN("color_temperature out of bound =%d", color_temperature);
		return -1;
	}
...
int get_lcd_timing(int lcd_node, unsigned long arg)
{
	...
	if (reg == NULL) {
		pr_err("pq get malloc failed!\n");
		return -1;
	}
	if (sizeof(struct lcd_timing) != ubuffer[3]) {
		pr_err("Transfer data size mismatch\n");
		return -1;
	}
