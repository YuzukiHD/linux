...
//		disp_al_manager_set_rcq_update(mgr->disp, 0);
		...
		if (dispdev->is_in_safe_period && dispdev->is_in_safe_period(dispdev)) {
			...
			__wrn("rcq unprotect not at safe region!\n");
		}
...
#if IS_ENABLED(CONFIG_AW_DISP2_COMPOSER) && RTMX_USE_RCQ
...
static s32 disp_mgr_clk_enable(struct disp_manager *mgr)
{
	...
	if (mgrp->rst_sys) {
		...
		if (ret) {
			DE_WRN("%s(%d): reset_control_deassert for rst_sys failed\n", __func__, __LINE__);
			return ret;
		}
	}
	...
	if (mgrp->rst_video_out) {
		...
		if (ret) {
			DE_WRN("%s(%d): reset_control_deassert for rst_video_out failed, ret=%d\n", __func__, __LINE__, ret);
			return ret;
		}
	}
			  ...
			  unsigned int layer_num)
{
	...
	for (i = 0; i < layer_num; i++) {
		if (config[i].info.mode != LAYER_MODE_BUFFER_FORCE) {
			DE_WRN("force_set_layer_config2 must set LAYER_MODE_BUFFER_FORCE!\n");
			return -EINVAL;
		}
	}
