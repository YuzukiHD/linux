	...
	struct disp_capture *capture, bool protect)
{
	...
	if (disp_feat_is_using_wb_rcq(capture->disp) > 1)
		return 0;
...
static s32 disp_capture_clk_enable(struct disp_capture *cptr)
{
	...
	if (cptrp->rst_sys) {
		ret = reset_control_deassert(cptrp->rst_sys);
		if (ret) {
			DE_WRN("%s: reset_control_deassert for rst_sys failed\n", __func__);
			return ret;
		}
	}
...
static s32 disp_capture_clk_disable(struct disp_capture *cptr)
{
	...
	if (cptrp->rst_sys) {
		ret = reset_control_assert(cptrp->rst_sys);
		if (ret) {
			DE_WRN("%s: reset_control_assert for rst_sys failed\n", __func__);
			return ret;
		}
	}
			...
			struct disp_capture_info *info)
{
	...
	if (disp_feat_is_using_wb_rcq(cptr->disp)) {
		...
		if (disp_feat_is_using_wb_rcq(cptr->disp) == 2) {
			if (mgr->reg_protect)
				...
				struct disp_capture_info2 *info)
{
	...
	*/
	...
	if (disp_feat_is_using_wb_rcq(cptr->disp)) {
		/* trigger writeback for the first frame after disp_capture_start() */
		if (rcq_cptr_wq.rcq_state < 1) {
			rcq_cptr_wq.rcq_state = 1;
			rcq_cptr_wq.cptr = cptr;
			schedule_work(&rcq_cptr_wq.wq);
		}
	}
...
s32 disp_init_capture(struct disp_bsp_init_para *para)
{
	...
	for (disp = 0; disp < num_screens; disp++) {
		...
		capturep->rst_sys = para->rst_bus_de_sys[disp];
