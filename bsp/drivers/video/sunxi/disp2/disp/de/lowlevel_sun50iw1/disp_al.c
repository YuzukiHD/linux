...
int disp_init_al(struct disp_bsp_init_para *para)
{
	...
	for (i = 0; i < DEVICE_NUM; i++) {
		if (para->boot_info[i].sync == 1) {
			u32 disp = para->boot_info[i].disp;
			u32 chn, layer_id;
			...
			al_priv.output_type[disp_device] = para->boot_info[i].type;
			al_priv.output_mode[disp_device] = para->boot_info[i].mode;
			al_priv.tcon_index[disp_device] = 0;
#if defined(SUPPORT_HDMI)
			al_priv.tcon_index[disp_device] = (para->boot_info[i].type == DISP_OUTPUT_TYPE_HDMI)?1:al_priv.tcon_index[disp_device];
#endif
#if defined(SUPPORT_TV)
			al_priv.tcon_index[disp_device] = (para->boot_info[i].type == DISP_OUTPUT_TYPE_TV)?1:al_priv.tcon_index[disp_device];
#endif
