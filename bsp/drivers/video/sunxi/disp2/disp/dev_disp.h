...
struct disp_init_para {
	...
	/* for hwc */
	unsigned int disp_device_num;
	unsigned int logic_id[8];
	unsigned int de_id[8];
	char type[8][16];
	unsigned int framebuffer_width[8];
	unsigned int framebuffer_height[8];
	unsigned int dpix[8];
	unsigned int dpiy[8];
...
s32 disp_edp_drv_init(int dispdev_id);
s32 disp_edp_drv_deinit(int dispdev_id);
