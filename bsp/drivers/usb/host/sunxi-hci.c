...
void usb_phyx_res_cal(__u32 usbc_no, bool enable)
{
	...
	|| IS_ENABLED(CONFIG_ARCH_SUN8IW18) || IS_ENABLED(CONFIG_ARCH_SUN55IW3)
...
static int open_clock(struct sunxi_hci_hcd *sunxi_hci, u32 ohci)
{
	...
	usb_hci_utmi_phy_tune(sunxi_hci, SUNXI_TX_PREEMPAMP_TUNE, SUNXI_TX_PREEMPAMP_TUNE_OFFSET, 0x1);
#endif
	...
	if (!sunxi_hci->clk_is_open) {
		...
		if (sunxi_hci->clk_hosc) {
			...
			if (ret) {
				dev_err(&sunxi_hci->pdev->dev, "enable clk_hosc err, return %d\n", ret);
		...
		struct sunxi_hci_hcd *sunxi_hci)
{
	...
	if (ret) {
		DMSG_PRINT("get gma340-oe is fail, %d\n", -ret);
		sunxi_hci->gma340_oe_gpio_valid = 0;
	} else {
		/* get gma340-oe gpio */
		sunxi_hci->gma340_oe_gpio_set.gpio =
...
int init_sunxi_hci(struct platform_device *pdev, int usbc_type)
{
	...
	if (sunxi_hci->thread_scan_flag) {
		...
		if (IS_ERR_OR_NULL(ts_scan)) {
			dev_err(&pdev->dev, "ERR: create sunxi_hci_scan_thread kthread failed\n");
			return -EINVAL;
		}
