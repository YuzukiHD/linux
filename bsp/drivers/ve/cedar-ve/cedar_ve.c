...
#define VE_LOGD(fmt, arg...) printk(KERN_DEBUG"VE:<%s:%u> "fmt"\n", __func__, __LINE__, ##arg)
#define VE_LOGI(fmt, arg...) printk(KERN_INFO"VE:<%s:%u> "fmt"\n", __func__, __LINE__, ##arg)
#define VE_LOGW(fmt, arg...) printk(KERN_WARNING"VE:<%s:%u> "fmt"\n", __func__, __LINE__, ##arg)
#define VE_LOGE(fmt, arg...) printk(KERN_ERR"VE:<%s:%u> "fmt"\n", __func__, __LINE__, ##arg)
...
static long setup_ve_freq(int ve_freq)
{
	...
	VE_LOGV("VE real_freq = %ld MHz", ret/1000000);
	return ret;
}
...
static void remove_case_load_param_by_process_id_sun55iw3(u32 cur_process_channel_id)
{
	...
	for (i = 0; i < MAX_VE_LOAD_PARAM_CHANNEL; i++) {
		if (cedar_devp->load_infos[i].is_used == 1
			...
			VE_LOGD("remove case load when call cedardev_release, i = %d, process_id = %d", i, cur_process_channel_id);
...
static int update_case_load_param_sun55iw3(struct ve_case_load_param *cur_load_param, u32 cur_process_channel_id)
{
	...
	VE_LOGV("update load info: w = %d, h = %d, f = %d, codec_format = %d, remove = %d, thread_id = %d, process_id = %d",
	...
	if (i < MAX_VE_LOAD_PARAM_CHANNEL)
		...
		if (i < MAX_VE_LOAD_PARAM_CHANNEL)
			...
			VE_LOGW("cannot find empty load_info, may be channels overflow %d", MAX_VE_LOAD_PARAM_CHANNEL);
			spin_unlock_irqrestore(&cedar_devp->lock, flags);
			return -1;
		}
	}
...
static int adjust_ve_freq_by_case_load_sun55iw3(void)
{
	...
	for (i = 0; i < MAX_VE_LOAD_PARAM_CHANNEL; i++) {
		...
		if (cur_load_info->is_used == 1) {
			VE_LOGV("i[%d]: w = %d, h = %d, f = %d, codec_format = %d, remove = %d, thread_id = %d, process_id = %d",
	...
	VE_LOGV("dec_ch_num = %d, enc_ch_num = %d, had_diff_codec_ch = %d, cur_format = %d, total_p = %u",
	...
	for (i = 0; i < array_size; i++) {
		VE_LOGV("i = %d, format = %d, start_p = %d, end_p = %d, ve_freq = %d", i,
	...
	if (i >= array_size) {
		VE_LOGW("cannot match performance info, it maybe wrong, i = %d, array_size = %d", i, array_size);
		return 0;
	}
		...
		case IOCTL_SET_VE_FREQ:
			{
				...
				VE_LOGD("VE setup_freq = %d MHz", arg_rate);
...
static int cedardev_init(struct platform_device *pdev)
{
	...
	VE_LOGD("LINUX_VERSION_CODE:%d", LINUX_VERSION_CODE);
	...
	if (!IS_ERR(cedar_devp->regulator)) {
		...
		VE_LOGD("ve vol = %d mv", cedar_devp->voltage);
	} else {
