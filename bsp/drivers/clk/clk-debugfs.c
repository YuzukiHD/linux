...
static int sunxi_rst_get_names(void)
{
	...
	if (testclk_priv.rst_count > 0) {
		...
		if (ret < 0) {
			pr_err("fail to get reset names\n");
			return -ENODATA;
		}
	}
	...
	if (start == 1) {
		...
		if (command < 9) {
			...
			if (!testclk_priv.reset_dev) {
				pr_err("not support rst test, please check if SUNXXIWXX_TEST_CCU is enabled\n");
				return;
			}

			reset = devm_reset_control_get(testclk_priv.reset_dev, testclk_priv.name);
			if (IS_ERR_OR_NULL(reset)) {
				pr_err("get reset control failed\n");
				return;
			}
		}
