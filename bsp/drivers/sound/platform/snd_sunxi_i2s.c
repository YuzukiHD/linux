...
static int sunxi_i2s_set_ch_map(struct sunxi_i2s *i2s, unsigned int channels)
{
	...
	for (i = 0; i < 4; i++) {
		if (dts->rx_pin[i]) {
			regmap_write(regmap, SUNXI_I2S_RXCHMAP0, dts->rx_pin_map[i][3]);
			regmap_write(regmap, SUNXI_I2S_RXCHMAP1, dts->rx_pin_map[i][2]);
			regmap_write(regmap, SUNXI_I2S_RXCHMAP2, dts->rx_pin_map[i][1]);
			regmap_write(regmap, SUNXI_I2S_RXCHMAP3, dts->rx_pin_map[i][0]);
		}
	}
				 ...
				 unsigned int *val)
{
	...
	case SUNXI_I2S_DAI_SLOT_WIDTH:
		*val = i2s_dai_fmt->slot_width;
		break;
	default:
		SND_LOG_ERR("unsupport dai fmt sel %d\n", dai_fmt_sel);
		return -EINVAL;
	}
...
static int sunxi_i2s_dai_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_DSP_B:
		...
		break;
	default:
		SND_LOG_ERR("dai_mode setting failed\n");
		return -EINVAL;
	}
...
static void snd_sunxi_dts_params_init(struct platform_device *pdev, struct sunxi_i2s_dts *dts)
{
	...
	char rx_pin_map0_str[16] = "", rx_pin_map1_str[16] = "";
	char rx_pin_map2_str[16] = "", rx_pin_map3_str[16] = "";
	struct device_node *np = pdev->dev.of_node;
	...
	for (i = 0; i < tx_pin_size; i++) {
		...
		if (temp_val > 3) {
			SND_LOG_WARN("tx-pin[%d] config invalid\n", i);
			continue;
		}
		if (ret < 0) {
			dts->tx_pin[temp_val] = false;
			SND_LOG_WARN("tx-pin[%d] config missing\n", i);
		} else {
	...
	for (i = 0; i < rx_pin_size; i++) {
		...
		if (temp_val > 3) {
			SND_LOG_WARN("rx-pin[%d] config invalid\n", i);
			continue;
		}
		if (ret < 0) {
			dts->rx_pin[temp_val] = false;
			SND_LOG_WARN("rx-pin[%d] config missing\n", i);
		} else {
			dts->rx_pin[temp_val] = true;
		}

		/* temp_val length shounld be 1 char */
		snprintf(rx_pin_map0_str, 13, "rx-pin%u-map0", temp_val);
		snprintf(rx_pin_map1_str, 13, "rx-pin%u-map1", temp_val);
		snprintf(rx_pin_map2_str, 13, "rx-pin%u-map2", temp_val);
		snprintf(rx_pin_map3_str, 13, "rx-pin%u-map3", temp_val);
		ret = of_property_read_u32(np, rx_pin_map0_str, &(dts->rx_pin_map[temp_val][0]));
		if (ret < 0) {
			SND_LOG_DEBUG("%s get failed! using SDI0\n", rx_pin_map0_str);
			dts->rx_pin_map[temp_val][0] = 0x03020100;
		}
		ret = of_property_read_u32(np, rx_pin_map1_str, &(dts->rx_pin_map[temp_val][1]));
		if (ret < 0) {
			SND_LOG_DEBUG("%s get failed! using SDI0\n", rx_pin_map1_str);
			dts->rx_pin_map[temp_val][1] = 0x07060504;
		}
		ret = of_property_read_u32(np, rx_pin_map2_str, &(dts->rx_pin_map[temp_val][2]));
		if (ret < 0) {
			SND_LOG_DEBUG("%s get failed! using SDI0\n", rx_pin_map2_str);
			dts->rx_pin_map[temp_val][2] = 0x0B0A0908;
		}
		ret = of_property_read_u32(np, rx_pin_map3_str, &(dts->rx_pin_map[temp_val][3]));
		if (ret < 0) {
			SND_LOG_DEBUG("%s get failed! using SDI0\n", rx_pin_map3_str);
			dts->rx_pin_map[temp_val][3] = 0x0F0E0D0C;
		}
	}
...
static void snd_sunxi_dump_version(void *priv, char *buf, size_t *count)
{
	...
	if (!i2s) {
		SND_LOG_ERR("priv to i2s failed\n");
		return;
	}
	if (i2s->pdev)
...
static int sunxi_i2s_dev_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(i2s)) {
		SND_LOG_ERR("alloc sunxi_i2s failed\n");
		ret = -ENOMEM;
