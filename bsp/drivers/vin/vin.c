...
static void vin_md_set_power(struct vin_md *vind, int on)
{
...
#if defined (CONFIG_ARCH_SUN50IW10P1) || defined (CONFIG_ARCH_SUN55IW3) || defined (CONFIG_ARCH_SUN55IW3)
			cmb_phy_top_enable();
	...
	if (on) {
		...
		csic_isp_bridge_enable(vind->id);
#endif
#if defined (CONFIG_ARCH_SUN50IW10) || defined (CONFIG_ARCH_SUN55IW3) || defined (CONFIG_ARCH_SUN60IW1)
...
#elif defined (CONFIG_ARCH_SUN55IW3) || defined (CONFIG_ARCH_SUN60IW1)
...
static int vin_probe(struct platform_device *pdev)
{
	...
	vind->id = pdev->id;
	vind->pdev = pdev;
#if defined CONFIG_VIN_INIT_MELIS && !defined RV_RUN_CAR_REVERSE
