			...
			struct mrg_int_ch_cfg *cfg)
{
	...
	for (i = 0; i < 10; i++) {
		if (GET_BIT(bk_ch_sel, i)) {
			if (i < 8)
				vind->bk_intpool.mask_cfg0 |= 1 << i;
			else
				vind->bk_intpool.mask_cfg0 |= 1 << (4 * (i - 8) + 8);
		}
	}
	/* cfg1 */
	for (i = 10; i < 24; i++) {
		if (GET_BIT(bk_ch_sel, i)) {
			if (i < 12)
				vind->bk_intpool.mask_cfg1 |= 1 << (4 * (i - 10));
			else if (i < 20)
				vind->bk_intpool.mask_cfg1 |= 1 << (i - 4);
			else if (i < 24)
				vind->bk_intpool.mask_cfg1 |= 1 << (4 * (i - 20) + 16);
		}
	}
#endif
	return 0;
...
#if IS_ENABLED(CONFIG_AW_DMC_DEVFREQ) && (defined CONFIG_ARCH_SUN50IW10 || !defined CSIC_SDRAM_DFS)
