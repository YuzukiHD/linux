...
{
	struct v4l2_dv_timings timings;
	//struct sensor_info *info = to_state(vsd);
	//struct sensor_indetect *sensor_indet = &info->sensor_indet;
	//mutex_lock(&sensor_indet->detect_lock);
	lt7911d_get_detected_timings(vsd, &timings);
...
{
	struct sensor_indetect *sensor_indet = container_of(work, struct sensor_indetect, sensor_work);
...
static irqreturn_t sensor_det_irq_func(int irq, void *priv)
{
	...
	schedule_work(&sensor_indet->sensor_work);
...
static int sensor_det_init(struct v4l2_subdev *sd)
{
	...
	INIT_WORK(&sensor_indet->sensor_work, sensor_det_work);
...
static void sensor_det_exit(struct v4l2_subdev *sd)
{
	...
	cancel_work_sync(&sensor_indet->sensor_work);
	if (sensor_indet->hotplug_det_irq > 0) {
