...
static inline int _set_dsu_clk_only(struct device *dev, struct clk *clk,
	unsigned long freq)
{
	int ret;
	...
	unsigned long little_core_next_freq)
{
	...
	if (IS_ERR(opp)) {
		pr_err("Failed to recommended opp\n");
...
static int set_dsufreq_scaling_down(struct cpufreq_policy *policy, unsigned long freq)
{
	...
	if (!dsufreq_dev) {
		pr_notice("cluster dev not init\n");
		return -ENODEV;
	}
...
static int dsu_init_freq_table(struct sunxi_dsufreq_dev *dsufreq_dev)
{
...
#endif
	if (ret) {
		dev_err(dev, "failed to set OPP regulator\n");
		return ret;
	}
	...
	if (strlen(opp_table_name)) {
		...
		if (IS_ERR(opp_table)) {
			...
			dev_err(dev, "Failed to set prop name, use default vf\n");
			return ret;
		}
...
static int sunxi_dsufreq_probe(struct platform_device *pdev)
{
	...
	for_each_possible_cpu(cpu) {
		...
		if (!policy) {
			dev_err(dev, "cpufreq cpu get failed\n");
			return -EPROBE_DEFER;
		}
		...
		if (policy != policy_bak) {
			...
			if (dsufreq_dev->policy_cnt > POLICY_NUM) {
				dev_warn(dev, "not support policy num\n");
				return -EINVAL;
			}
		}
	}
	...
	if (err) {
		dev_err(dev, "Fail init freq table\n");
		return err;
	}
	...
	if (err) {
		pr_err("failed to dsufreq class register, err = %d\n", err);
		return err;
	}
