...
u32 ss_hash_start(ss_hash_ctx_t *ctx, ss_aes_req_ctx_t *req_ctx, u32 len, u32 last)
{
	...
	ss_hash_iv_mode_set(1, task); /* default enabled */

#ifdef SS_HMAC_ENABLE
	if (CE_METHOD_IS_HMAC(req_ctx->type))
		task->comm_ctl &= ~(1 << CE_CTL_IV_MODE_SHIFT); /* hmac should disabled IV-mode to use default IV */
#endif
	...
	/* Multi-sg mode is enabled, this bit should be fixed to 1 */
