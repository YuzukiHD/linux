...
static int sunxi_msgbox_suspend_noirq(struct device *dev)
{
...
static int sunxi_msgbox_resume_noirq(struct device *dev)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct sunxi_msgbox *chip = platform_get_drvdata(pdev);
...
static const struct dev_pm_ops sunxi_msgbox_dev_pm_ops = {
	.suspend_noirq = sunxi_msgbox_suspend_noirq,
	.resume_noirq = sunxi_msgbox_resume_noirq,
};
