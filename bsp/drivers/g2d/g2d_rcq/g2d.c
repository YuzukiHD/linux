...
static int g2d_clock_prepare(const __g2d_info_t *info)
{
	...
	if (ret != 0)
		pr_err("[G2D] clock prepare error\n");
...
long g2d_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case G2D_CMD_BITBLT_H:
		...
		g2d_blt_h *blit_para = (g2d_blt_h *)kmalloc(sizeof(g2d_blt_h),
							    GFP_KERNEL);
		if (!blit_para) {
			pr_err("[G2D]blit_para kmalloc failed!\n");
			ret = -EFAULT;
			goto err_noput;
		}
	...
	case G2D_CMD_BLD_H:{
			...
			if (dbg_info) {
				printk("src_image[0]\n");
				print_image(&bld_para.src_image[0]);
				printk("src_image[1]\n");
				print_image(&bld_para.src_image[1]);
				printk("dst_image\n");
				print_image(&bld_para.dst_image);
			}
...
static int g2d_attach_pd(struct device *dev, const char *values_of_power_domain_names[], int array_size)
{
	...
	for (i = 0; i < array_size; i++) {
		...
		link = device_link_add(dev, pd_dev,
				...
				DL_FLAG_PM_RUNTIME);
		if (!link) {
			pr_err("Failed to add device_link to %s.\n", values_of_power_domain_names[i]);
			return -EINVAL;
		}
	}
	return 0;
