					 ...
					 struct regulator *supply)
{
	...
	case BIAS_VOLTAGE_PIO_POW_MODE_CTL:
		...
		reg = readl(pctl->membase + sunxi_pinctrl_hw_info[hw_type].power_mode_sel_reg);
		reg &= ~(1 << bank);
		writel(reg | val << bank, pctl->membase + sunxi_pinctrl_hw_info[hw_type].power_mode_sel_reg);
		...
		raw_spin_lock_irqsave(&pctl->lock, flags);
		reg = readl(pctl->membase + sunxi_pinctrl_hw_info[hw_type].power_mode_ctrl_reg);
		reg &= ~BIT(bank);
		writel(reg | val << bank, pctl->membase + sunxi_pinctrl_hw_info[hw_type].power_mode_ctrl_reg);
		raw_spin_unlock_irqrestore(&pctl->lock, flags);
		return 0;
...
static void sunxi_power_switch_pf(struct sunxi_pinctrl *pctl, unsigned pin, struct regulator *supply)
{
	u32 val, reg;
	int cur_vol, tar_vol;
	unsigned long flags;
	...
	tar_vol = regulator_get_voltage(supply);
	if (tar_vol < 0)
		return;

	cur_vol = readl(pctl->membase + sunxi_pinctrl_hw_info[hw_type].pio_pow_ctrl_reg);
	cur_vol = cur_vol == 0 ? 1800000 : 3300000;

	if (cur_vol < tar_vol) {
		if (sunxi_pinctrl_hw_info[hw_type].power_mode_flag == 1)
			val = 1;
		else
			val = 0;
		...
		val = 1;
		writel(val, pctl->membase + sunxi_pinctrl_hw_info[hw_type].pio_pow_ctrl_reg);
			   ...
			   unsigned long *configs, unsigned num_configs)
{
	...
	for (i = 0; i < num_configs; i++) {
		...
		case PIN_CONFIG_POWER_SOURCE:
			 ...
			 */
			sunxi_power_switch_pf(pctl, pin,
						(arg == 1800) ?
						s_reg->regulator :
			     ...
			     unsigned group)
{
	...
	if (pctl->pad_addr[0].addr && !strncmp(func->name, pctl->pad_addr[0].name, strlen(func->name))) {
		if (g->pin > PL_BASE) {
			pr_debug("i2s0 change to r_pio!\n");
			writel(regval & BIT(index), pctl->pad_addr[0].addr);
		} else {
			pr_debug("i2s0 change to pio!\n");
			writel(regval | BIT(index), pctl->pad_addr[0].addr);
		}
	}
	...
	if (pctl->pad_addr[1].addr && !strncmp(func->name, pctl->pad_addr[1].name, strlen(func->name))) {
		if (g->pin > PL_BASE) {
			pr_debug("dmic change to r_pio!\n");
			writel(regval & BIT(index), pctl->pad_addr[1].addr);
		} else {
			pr_debug("dmic change to pio!\n");
			writel(regval | BIT(index), pctl->pad_addr[1].addr);
		}
	}
#endif
