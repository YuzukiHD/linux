...
#ifdef CONFIG_AW_SPINAND_PSTORE_PANIC_WRITE
/* write when on panic */
		...
		size_t len, size_t *retlen, const u_char *buf)
{
	...
	pr_info("calling panic write: to 0x%llx datalen %lu\n", to,
			(unsigned long)len);
	...
	aw_spinand_for_each_req(spinand, to, &ops, &req) {
		...
		ret = chip_ops->write_page(chip, &req);
		if (ret < 0) {
			pr_err("panic write single page failed: block %d, page %d, ret %d\n",
					req.block, req.page, ret);
			break;
		}
