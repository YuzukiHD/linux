...
static int sunxi_spif_clk_init(struct sunxi_spif *sspi)
{
	...
	if (ret) {
		dev_err(&sspi->pdev->dev, "Couldn't enable bus clock\n");
		goto err2;
	}
...
static void sunxi_spif_config_tc(struct sunxi_spif *sspi)
{
	if (sspi->data->sample_mode != SAMP_MODE_DL_DEFAULT) {
		...
		double_clk_flag = 0;
		dev_info(&sspi->pdev->dev, "[spi-flash%d] working clk %u\n",
				sspi->data->bus_num,
...
static int sunxi_spif_hw_init(struct sunxi_spif *sspi)
{
	...
	if (err) {
		dev_err(&sspi->pdev->dev, "sunxi_spif regulator enable error\n");
		goto err0;
	}

	err = sunxi_spif_pinctrl_init(sspi);
	if (err) {
		dev_err(&sspi->pdev->dev, "sunxi_spif pinctrl init error\n");
		goto err1;
	}

	err = sunxi_spif_clk_init(sspi);
	if (err) {
		dev_err(&sspi->pdev->dev, "sunxi_spif clk init error\n");
		goto err2;
	}
...
static int sunxi_spif_resource_get(struct sunxi_spif *sspi)
{
	...
	if (IS_ERR(sspi->mclk)) {
		dev_err(&sspi->pdev->dev, "spif unable to acquire bus clock\n");
		return -EINVAL;
	}
...
static int sunxi_spif_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < DESC_PER_DISTRIBUTION_MAX_NUM; i++) {
		...
		dev_dbg(&pdev->dev, "dma descriptor phys addr is %llx\n", sspi->desc_phys[i]);
	}
	...
	if (err) {
		dev_err(&pdev->dev, " sunxi spif nor register error:%d\n", err);
		goto err6;
	}

	dev_info(&pdev->dev, "probe succeed (Version %s)\n", SUNXI_SPIF_MODULE_VERSION);
	return 0;
...
static int sunxi_spif_resume(struct device *dev)
{
	...
	if (ret) {
		dev_err(&pdev->dev, "spif resume error\n");
		return ret;
	}
