									...
									struct sunxi_spi_slave_head *head, u8 *buf)
{
	...
	if (cache->size < head->addr) {
		dev_err(dev, "Set data addr over range\n");
		return 0;
	}

	if (cache->size < head->addr + head->len) {
		real_size = cache->size - head->addr;
		dev_err(dev, "Write size %d over range, some of data will be lost, real size to write is %d\n",
				head->len, real_size);
	}
										...
										struct sunxi_spi_slave_head *head, u8 *buf)
{
	...
	if (cache->size < head->addr) {
		dev_err(dev, "Get data addr over range\n");
		return 0;
	}

	if (cache->size < head->addr + head->len) {
		real_size = cache->size - head->addr;
		dev_err(dev, "Read size %d over range, some of data will be lost, real size to read is %d\n",
			head->len, real_size);
	}
...
static void sunxi_spi_slave_test_complete(void *arg)
{
	...
	case SUNXI_OP_HEAD:
		dev_dbg(dev, "sunxi slave get package head\n");
		break;
	case SUNXI_OP_WRITE:
		dev_dbg(dev, "sunxi slave get package operation write, recv read buffer\n");
...
terminate:
	...
	dev_info(dev, "slave transfer terminate\n");
...
static int sunxi_spi_slave_test_submit(struct sunxi_spi_slave_test *slave)
{
	...
	if (!sunxi_spi_slave_has_ptk_head(pkt_head)) {
		...
		case SUNXI_OP_READ:
			...
			dev_dbg(dev, "sunxi slave get package operation read, send write buffer\n");
			break;
		default:
			dev_err(dev, "unknown op code %d, wait revice from master\n", pkt_head->op_code);
			sunxi_spi_slave_head_data_clear(slave->frame.data, sizeof(slave->frame.data));
			pkt_head->op_code = SUNXI_OP_HEAD;
			slave->xfer.rx_buf = slave->frame.data;
			slave->xfer.len = sizeof(slave->frame.data);
...
static ssize_t sunxi_slave_abort_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	dev_info(dev, "slave transfer abort\n");
...
static ssize_t sunxi_slave_submit_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (slave->status != SUNXI_SPI_SLAVE_IDLE) {
		dev_warn(dev, "slave transfer isn't idle\n");
		return count;
	}

	dev_info(dev, "slave transfer submit\n");
...
static int sunxi_spi_slave_test_probe(struct spi_device *spi)
{
	...
	if (ret) {
		...
		dev_warn(dev, "failed to get slave-cachesize, use default value %d\n", slave->cache.size);
	} else if (slave->cache.size > SALVE_CACHE_MAX) {
		slave->cache.size = SALVE_CACHE_MAX;
		dev_err(dev, "slave-cachesize too big, use default value %d\n", slave->cache.size);
	}
	...
	dev_info(dev, "slave cachesize %d\n", slave->cache.size);
