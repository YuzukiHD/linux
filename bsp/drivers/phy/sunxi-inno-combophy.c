...
static void sunxi_combphy_pcie_phy_24M(struct sunxi_combphy *combphy)
{
	u32 val;
	/* set to output 100M differential clock */
	...
	writel(0x1f140a0a, combphy->phy_clk + 0x1418);
}
...
{
	if (combphy->clk_freq_24M)
		sunxi_combphy_pcie_phy_24M(combphy);
	else if (combphy->clk_freq_100M)
				     ...
				     struct sunxi_combphy *combphy)
{
	...
	combphy->clk_freq_24M = false;
	combphy->clk_freq_100M = false;
	...
	if (device_property_read_bool(&pdev->dev, "clk-freq-24M")) {
		combphy->clk_freq_24M = true;
	}

	if (device_property_read_bool(&pdev->dev, "clk-freq-100M")) {
		combphy->clk_freq_100M = true;
	}
