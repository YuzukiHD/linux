...
struct sunxi_uart_port {
	...
	struct sunxi_uart_register reg;
	struct sunxi_uart_pdata *pdata;
	struct sunxi_uart_dma *dma;
...
#if IS_ENABLED(CONFIG_ARCH_SUN55IW3) || IS_ENABLED(CONFIG_ARCH_SUN50IW10) || IS_ENABLED(CONFIG_ARCH_SUN8IW11)
...
extern int sunxi_uart_init_dma_tx(struct sunxi_uart_port *uart_port);
extern void dma_tx_callback(void *data);
extern int sunxi_uart_start_dma_tx(struct sunxi_uart_port *uart_port);
extern void sunxi_uart_release_dma_rx(struct sunxi_uart_port *uart_port);
extern int sunxi_uart_init_dma_rx(struct sunxi_uart_port *uart_port);
extern int sunxi_uart_start_dma_rx(struct sunxi_uart_port *uart_port);
