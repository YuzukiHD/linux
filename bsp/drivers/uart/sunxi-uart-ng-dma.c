...
void sunxi_uart_dma_rx_callback(void *arg)
{
	...
	if (!uart_dma->rx_dma_used || !port->state->port.tty) {
		SERIAL_DBG("!uart_dma->rx_dma_used || !port->state->port.tty\n");
		return;
	}
	SERIAL_DBG("uart_rx callback\n");
	/* TODO: add trace event */
	dmaengine_tx_status(uart_dma->dma_chan_rx,
			uart_dma->rx_cookie, &state);

	SERIAL_DBG("dma left data:%d\n", state.residue);
	...
	count = CIRC_CNT_TO_END(uart_dma->rb_head, uart_dma->rb_tail,
			uart_dma->rb_size);
	SERIAL_DBG("total data:%d count data:%d\n", total, count);
	if (!total) {
		SERIAL_DBG("dma data is zero\n");
		return;
	}
	...
	if (unlikely(flip != total))
		pr_err("flip data cnt not equel to rev data\n");
...
int sunxi_uart_init_dma_tx(struct sunxi_uart_port *uart_port)
{
	...
	dev_info(uart_port->port.dev, "sunxi_uart_init_dma_tx begin\n");

	if (!uart_dma) {
		dev_info(uart_port->port.dev, "sunxi_uart_init_dma_tx fail\n");
		return -1;
	}

	if (uart_dma->tx_dma_inited) {
		dev_info(uart_port->port.dev, "uart_dma->tx_dma_inited\n");
		return 0;
	}
	...
	dev_info(port->dev, "sunxi_uart_init_dma_tx sucess\n");
	return 0;
}
...
void dma_tx_callback(void *data)
{
	...
	dev_info(port->dev, "dma_tx_callback\n");
...
int sunxi_uart_start_dma_tx(struct sunxi_uart_port *uart_port)
{
	...
	return 1;
err_out:
	dev_info(uart_port->port.dev, "-sunxi_uart_start_dma_tx-error-\n");
	return -1;
}
...
int sunxi_uart_init_dma_rx(struct sunxi_uart_port *uart_port)
{
	...
	SERIAL_DBG("sunxi_uart_init_dma_rx\n");

	if (!uart_dma) {
		dev_info(port->dev, "sunxi_uart_init_dma_rx: port fail\n");
		return -1;
	}

	if (uart_dma->rx_dma_inited) {
		SERIAL_DBG("uart_dma->rx_dma_inited\n");
		return 0;
	}
	...
	dev_info(port->dev, "sunxi_uart_init_dma_rx sucess\n");
	return 0;
}
...
int sunxi_uart_start_dma_rx(struct sunxi_uart_port *uart_port)
{
	...
	SERIAL_DBG("enter  sunxi_uart_start_dma_rx\n");

	if (!uart_dma->use_dma) {
		SERIAL_DBG("uart_dma->use_dma not set\n");
		return 0;
	}

	if (uart_dma->rx_dma_used == 1) {
		SERIAL_DBG("uart_dma->rx_dma_used == 1\n");
		return 0;
	}

	if (-1 == sunxi_uart_init_dma_rx(uart_port)) {
		dev_info(uart_port->port.dev, "sunxi_uart_init_dma_rx error!\n");
		return -1;
	}
...
int sunxi_uart_init_dma(struct sunxi_uart_port *uart_port)
{
	if (uart_port->dma->use_dma & RX_DMA) {
		...
		if (!uart_port->dma->rx_buffer) {
			dev_err(uart_port->port.dev,
				"dmam_alloc_coherent dma_rx_buffer fail\n");
			return -ENOMEM;
		} else {
			dev_info(uart_port->port.dev,
	...
	if (uart_port->dma->use_dma & TX_DMA) {
		...
		if (!uart_port->dma->tx_buffer) {
			...
			dev_info(uart_port->port.dev, "dma_tx_buffer %p\n",
						uart_port->dma->tx_buffer);
			dev_info(uart_port->port.dev, "dma_tx_phy 0x%08x\n",
